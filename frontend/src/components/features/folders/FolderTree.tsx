...
const FolderNode: React.FC<FolderNodeProps> = ({ node, level, selectedFolderId, onFolderSelect }) => {
...
export const FolderTree: React.FC<FolderTreeProps> = ({ selectedFolderId, onFolderSelect }) => {
  ...
  if (error) {
    return <ErrorDisplay message={error.message ?? 'Failed to load folder tree'} />;
  }
