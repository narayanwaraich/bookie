...
const errorHandler = (
  ...
  if (statusCode >= 500 && !(err instanceof Prisma.PrismaClientKnownRequestError)) {
     logger.error(`[ErrorHandler] ${req.method} ${req.originalUrl} - Status: ${statusCode} - Message: ${err.message} - Stack: ${err.stack}`);
  } else {
     logger.warn(`[ErrorHandler] ${req.method} ${req.originalUrl} - Status: ${statusCode} - Message: ${err.message}` + (err.name ? ` (${err.name})` : ''));
  }
  ...
  if (err instanceof ZodError) {
    ...
    logger.warn(`[ErrorHandler] Zod Validation Error: ${JSON.stringify(errors)}`);
  } 
  ...
  else if (err.name === 'AuthError' || err.name === 'BookmarkError' || err.name === 'CollectionError' || 
           err.name === 'FolderError' || err.name === 'ImportExportError' || err.name === 'SyncError' || 
           err.name === 'TagError' || err.name === 'UserServiceError') 
  {
  ...
  // Handle specific Prisma known request errors
    ...
    switch (err.code) {
        case 'P2002': // Unique constraint violation
            ...
            message = `Record already exists or unique constraint failed` + (target ? ` on field(s): ${Array.isArray(target) ? target.join(', ') : target}` : '');
            break;
        case 'P2025': // Record not found (for update/delete)
            statusCode = 404; // Not Found
            message = err.meta?.cause as string || 'Required record not found'; // Use Prisma's cause if available
            break;
        ...
        default:
            ...
            logger.error(`[ErrorHandler] Unhandled Prisma Known Request Error Code: ${err.code}`, err);
            break;
    }
  }
  // Handle Prisma validation errors (e.g., incorrect data type format before DB)
    ...
    statusCode = 400; // Bad Request
    message = 'Database input validation error. Please check your data.';
    logger.warn(`[ErrorHandler] Prisma Validation Error: ${err.message}`);
  }
