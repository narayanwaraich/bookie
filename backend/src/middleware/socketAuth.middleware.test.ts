...
describe('Socket Auth Middleware', () => {
  ...
  const mockDbUser = { ...mockUserPayload, name: 'Socket Tester' /* other fields */ };
  ...
  it('should call next(error) if no token is provided', async () => {
    ...
    expect(mockNext).toHaveBeenCalledWith(expect.objectContaining({ message: 'Authentication error: Token not provided' }));
  });
  ...
  it('should call next(error) if token verification fails', async () => {
    ...
    expect(mockNext).toHaveBeenCalledTimes(1);
    expect(mockNext).toHaveBeenCalledWith(expect.any(Error));
    expect(mockNext).toHaveBeenCalledWith(expect.objectContaining({ message: 'Authentication error: Invalid token' }));
  });
  ...
  it('should call next(error) if user not found in DB', async () => {
    ...
    expect(mockNext).toHaveBeenCalledTimes(1);
    expect(mockNext).toHaveBeenCalledWith(expect.any(Error));
    expect(mockNext).toHaveBeenCalledWith(expect.objectContaining({ message: 'Authentication error: User not found' }));
  });
