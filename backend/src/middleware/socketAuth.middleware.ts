...
export const socketAuthMiddleware = async (socket: AuthenticatedSocket, next: (err?: Error) => void) => {
  logger.debug(`[Socket Auth]: Attempting authentication for socket ID: ${socket.id}`);
  try {
    ...
    if (!token) {
      logger.warn(`[Socket Auth]: Authentication failed for socket ${socket.id}. Reason: No token provided.`);
    ...
    if (!user) {
       logger.warn(`[Socket Auth]: Authentication failed for socket ${socket.id}. Reason: User ${decoded.id} not found.`);
       return next(new Error('Authentication error: User not found'));
    }
    if (!user.isActive) {
       logger.warn(`[Socket Auth]: Authentication failed for socket ${socket.id}. Reason: User ${decoded.id} is inactive.`);
    ...
    logger.info(`[Socket Auth]: User ${user.username} (ID: ${user.id}) authenticated successfully for socket ${socket.id}`);
    // Join the user-specific room for targeted emits
    socket.join(user.id); 
    logger.debug(`[Socket Auth]: Socket ${socket.id} joined room ${user.id}`);
    
    next(); // Authentication successful, allow connection

  } catch (error) {
    logger.error(`[Socket Auth]: Authentication failed for socket ${socket.id}:`, error);
    ...
    next(new Error('Authentication error: Could not verify credentials')); 
  }
};
