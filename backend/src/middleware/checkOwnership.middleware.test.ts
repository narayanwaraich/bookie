...
describe('Check Ownership Middleware', () => {
  ...
  beforeEach(() => {
    ...
    mockRequest = {
      ...
      user: { id: userId, email: 'owner@test.com', username: 'owner' },
    };
  ...
  describe('Folder Ownership', () => {
    ...
    it('should call next() if user owns the folder', async () => {
      mockPrisma.folder.findUnique.mockResolvedValue({ id: resourceId, userId: userId } as any);
      await middleware(mockRequest as AuthenticatedRequest, mockResponse as Response, mockNext);
      // Middleware now only selects userId
      expect(mockPrisma.folder.findUnique).toHaveBeenCalledWith({ where: { id: resourceId }, select: { userId: true } });
    ...
    it('should call res.status(404) if folder not found', async () => {
      ...
      expect(mockResponse.json).toHaveBeenCalledWith(expect.objectContaining({ message: 'Folder not found' }));
    ...
    it('should call res.status(403) if user does not own the folder', async () => {
      mockPrisma.folder.findUnique.mockResolvedValue({ id: resourceId, userId: otherUserId } as any);
      await middleware(mockRequest as AuthenticatedRequest, mockResponse as Response, mockNext);
      expect(mockPrisma.folder.findUnique).toHaveBeenCalledWith({ where: { id: resourceId }, select: { userId: true } });
      expect(mockResponse.status).toHaveBeenCalledWith(403);
      expect(mockResponse.json).toHaveBeenCalledWith(expect.objectContaining({ message: 'Permission denied: You do not own this resource.' }));
  ...
  describe('Tag Ownership', () => {
    ...
    it('should call next() if user owns the tag', async () => {
      mockPrisma.tag.findUnique.mockResolvedValue({ id: resourceId, userId: userId } as any);
      await middleware(mockRequest as AuthenticatedRequest, mockResponse as Response, mockNext);
      expect(mockPrisma.tag.findUnique).toHaveBeenCalledWith({ where: { id: resourceId }, select: { userId: true } });
    ...
    it('should call res.status(404) if tag not found', async () => {
      ...
      expect(mockResponse.json).toHaveBeenCalledWith(expect.objectContaining({ message: 'Tag not found' }));
      expect(mockNext).not.toHaveBeenCalled();
    });

    it('should call res.status(403) if user does not own the tag', async () => {
      mockPrisma.tag.findUnique.mockResolvedValue({ id: resourceId, userId: otherUserId } as any);
      await middleware(mockRequest as AuthenticatedRequest, mockResponse as Response, mockNext);
      expect(mockPrisma.tag.findUnique).toHaveBeenCalledWith({ where: { id: resourceId }, select: { userId: true } });
      expect(mockResponse.status).toHaveBeenCalledWith(403);
      expect(mockResponse.json).toHaveBeenCalledWith(expect.objectContaining({ message: 'Permission denied: You do not own this resource.' }));
  ...
  describe('Collection Ownership', () => { // Note: Middleware uses ownerId field
    const middleware = checkOwnership('collection');

    it('should call next() if user owns the collection', async () => {
      mockPrisma.collection.findUnique.mockResolvedValue({ id: resourceId, ownerId: userId } as any); // Use ownerId
      await middleware(mockRequest as AuthenticatedRequest, mockResponse as Response, mockNext);
      expect(mockPrisma.collection.findUnique).toHaveBeenCalledWith({ where: { id: resourceId }, select: { ownerId: true } });
      expect(mockNext).toHaveBeenCalledWith();
    });
    ...
    it('should call res.status(404) if collection not found', async () => {
      ...
      expect(mockPrisma.collection.findUnique).toHaveBeenCalledWith({ where: { id: resourceId }, select: { ownerId: true } });
      expect(mockResponse.status).toHaveBeenCalledWith(404);
      expect(mockResponse.json).toHaveBeenCalledWith(expect.objectContaining({ message: 'Collection not found' }));
      expect(mockNext).not.toHaveBeenCalled();
    });

    it('should call res.status(403) if user does not own the collection', async () => {
      mockPrisma.collection.findUnique.mockResolvedValue({ id: resourceId, ownerId: otherUserId } as any); // Use ownerId
      await middleware(mockRequest as AuthenticatedRequest, mockResponse as Response, mockNext);
      expect(mockPrisma.collection.findUnique).toHaveBeenCalledWith({ where: { id: resourceId }, select: { ownerId: true } });
      expect(mockResponse.status).toHaveBeenCalledWith(403);
      expect(mockResponse.json).toHaveBeenCalledWith(expect.objectContaining({ message: 'Permission denied: You do not own this resource.' }));
  ...
  describe('Invalid Resource Type', () => {
    it('should call res.status(500) for unsupported resource type', async () => {
      ...
      expect(mockResponse.json).toHaveBeenCalledWith(expect.objectContaining({ message: 'Internal server error: Invalid resource type for ownership check' }));
