...
describe('Error Handling Middleware', () => {
  ...
  it('should handle ZodError and return 400', () => {
    ...
    // Assert
    expect(mockLoggerError).toHaveBeenCalledWith('Validation Error:', error.errors);
  ...
  it('should handle PrismaClientKnownRequestError (Unique Constraint P2002) and return 409', () => {
    // Arrange
    const error = new Prisma.PrismaClientKnownRequestError('Unique constraint failed', { code: 'P2002', clientVersion: 'x.y.z', meta: { target: ['email'] } });

    // Act
    errorHandler(error, mockRequest as Request, mockResponse as Response, mockNext);

    // Assert
    expect(mockLoggerError).toHaveBeenCalledWith('Prisma Error:', error);
   ...
   it('should handle PrismaClientKnownRequestError (Not Found P2025) and return 404', () => {
    // Arrange
    const error = new Prisma.PrismaClientKnownRequestError('Record not found', { code: 'P2025', clientVersion: 'x.y.z' });
  ...
  it('should handle custom AppError (e.g., AuthError) and return its status code', () => {
    ...
    // Assert
    expect(mockLoggerError).toHaveBeenCalledWith('Application Error:', error);
   ...
   it('should handle generic Error and return 500', () => {
    ...
    // Assert
    expect(mockLoggerError).toHaveBeenCalledWith('Unhandled Error:', error);
  ...
  it('should not send response if headers already sent', () => {
    ...
    // Assert
    expect(mockLoggerError).toHaveBeenCalledWith('Error after headers sent:', error);
  ...
  it('should handle FolderError', () => {
    ...
    expect(jsonMock).toHaveBeenCalledWith({ success: false, message: 'Folder access denied' });
  });
  
   it('should handle BookmarkError', () => {
    const error = new BookmarkError('Bookmark validation failed', 400);
    errorHandler(error, mockRequest as Request, mockResponse as Response, mockNext);
    expect(statusMock).toHaveBeenCalledWith(400);
    expect(jsonMock).toHaveBeenCalledWith({ success: false, message: 'Bookmark validation failed' });
  });
