...
const checkOwnership = (resourceType: ResourceType) => {
  return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    ...
    if (!userId) {
        ...
        logger.error('[checkOwnership Middleware]: User ID not found on request object. Ensure protect middleware runs first.');
        return res.status(401).json({ success: false, message: 'Not authenticated' });
    }
    if (!resourceId) {
      logger.warn(`[checkOwnership Middleware]: Resource ID missing in request params for type ${resourceType}.`);
      return res.status(400).json({ success: false, message: 'Resource ID parameter is required' });
    }

    logger.debug(`[checkOwnership Middleware]: Checking ownership for user ${userId} on ${resourceType} ${resourceId}`);
    ...
    try {
      ...
      switch (resourceType) {
        ...
        default:
          ...
          logger.error(`[checkOwnership Middleware]: Invalid resource type specified: ${resourceType}`);
          return res.status(500).json({ success: false, message: 'Internal server error: Invalid resource type for ownership check' });
      }

      // If not the owner, deny access (as this middleware only checks ownership)
      if (!isOwner) {
        logger.warn(`[checkOwnership Middleware]: Access denied. User ${userId} is not the owner of ${resourceType} ${resourceId}.`);
        // Return 403 Forbidden, as the resource might exist but user lacks ownership permission
        return res.status(403).json({ success: false, message: 'Permission denied: You do not own this resource.' }); 
      }

      // Ownership confirmed
      logger.debug(`[checkOwnership Middleware]: Ownership confirmed for user ${userId} on ${resourceType} ${resourceId}.`);
      next(); // Proceed to the next middleware or route handler

    } catch (error) {
      logger.error(`[checkOwnership Middleware]: Database error checking ownership for ${resourceType} ${resourceId}:`, error);
