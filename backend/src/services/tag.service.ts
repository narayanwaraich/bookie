...
// --- Cache Keys ---
export const TAG_LIST_CACHE_KEY = (userId: string, queryParams: any) => `user:${userId}:tags:list:${JSON.stringify(queryParams)}`;
export const TAG_DETAIL_CACHE_KEY = (tagId: string) => `tag:${tagId}:details`;
export const TAG_BOOKMARKS_CACHE_KEY = (tagId: string, queryParams: any) => `tag:${tagId}:bookmarks:${JSON.stringify(queryParams)}`;
export const USER_TAG_LIST_PATTERN = (userId: string) => `user:${userId}:tags:list:*`;
 ...
 */
export const createTag = async (userId: string, data: CreateTagInput): Promise<Tag> => {
    ...
    try {
        ...
        logger.info(`Tag created successfully: ${newTag.id} for user ${userId}`);
        ...
        logger.error(`Error creating tag "${name}" for user ${userId}:`, error);
 ...
 */
export const getAllUserTags = async (userId: string, queryParams: z.infer<typeof getTagsQuerySchema>) => {
    ...
    const cachedResult = await cacheWrap(cacheKey, async () => {
        ...
        logger.debug(`Fetching tags (DB) for user ${userId} with limit: ${limit}, offset: ${offset}, sortBy: ${sortBy}, sortOrder: ${sortOrder}`);
        ...
        try {
            const [tags, totalCount] = await prisma.$transaction([
                prisma.tag.findMany({
                    where,
                    include: { _count: { select: { bookmarks: { where: { bookmark: { isDeleted: false } } } } } },
            ...
            logger.info(`Found ${tagsWithCount.length} tags (DB) (total: ${totalCount}) for user ${userId}`);
            ...
            logger.error(`Error fetching tags (DB) for user ${userId}:`, error);
            throw new TagError('Failed to fetch tags', 500);
        }
    });

    if (cachedResult === null) throw new TagError('Failed to fetch tags', 500);
    return cachedResult;
};
...
export const getTagById = async (tagId: string, userId: string) => {
     ...
     const cachedResult = await cacheWrap(cacheKey, async () => {
         ...
         try {
            ...
            if (!tag) {
                 logger.warn(`Tag ${tagId} not found or deleted for user ${userId}.`);
                 throw new TagError('Tag not found', 404);
            }
            const { _count, ...tagData } = tag;
            const tagWithCount = { ...tagData, bookmarkCount: _count?.bookmarks ?? 0 };
            logger.info(`Tag ${tagId} fetched successfully (DB) for user ${userId}`);
            return tagWithCount; 
        } catch (error) {
            logger.error(`Error fetching tag ${tagId} (DB) for user ${userId}:`, error);
            if (error instanceof TagError) throw error; 
            throw new TagError('Failed to fetch tag', 500); 
        }
     });

     if (cachedResult === null) throw new TagError('Failed to fetch tag', 500);
     // Re-check ownership on cached data
     if (cachedResult.userId !== userId) {
         logger.warn(`Cache access violation attempt: User ${userId} tried to access tag ${tagId} owned by ${cachedResult.userId}`);
 ...
 */
export const updateTag = async (tagId: string, userId: string, data: UpdateTagInput): Promise<Tag> => {
    const { name, color } = data;
    ...
    if (name && name !== existingTag.name) {
        ...
        if (duplicateTag) {
            throw new TagError('Another tag with this name already exists', 409); 
        }
    }
    ...
    try {
        ...
        logger.info(`Tag updated successfully: ${tagId} by user ${userId}`);
        ...
        emitToUser(getIO(), userId, SOCKET_EVENTS.TAG_UPDATED, updatedTag); // Use getIO()
        return updatedTag;
    } catch (error) {
        logger.error(`Error updating tag ${tagId} for user ${userId}:`, error);
...
export const deleteTag = async (tagId: string, userId: string): Promise<void> => {
    ...
    try {
        ...
        logger.info(`Tag soft deleted successfully: ${tagId} by user ${userId}`);
        ...
        emitToUser(getIO(), userId, SOCKET_EVENTS.TAG_DELETED, { id: tagId }); // Use getIO()
        
    } catch (error) {
        logger.error(`Error deleting tag ${tagId} for user ${userId}:`, error);
         if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2025') {
 ...
 */
export const getBookmarksByTag = async (tagId: string, userId: string, queryParams: z.infer<typeof getBookmarksByTagQuerySchema>) => {
     const cacheKey = TAG_BOOKMARKS_CACHE_KEY(tagId, queryParams);

     // Check if tag exists and belongs to the user *before* caching
    const tag = await prisma.tag.findFirst({ where: { id: tagId, userId, isDeleted: false } }); 
    if (!tag) {
         logger.warn(`Get bookmarks by tag failed: Tag ${tagId} not found or deleted for user ${userId}.`);
        throw new TagError('Tag not found', 404);
    }

     const cachedResult = await cacheWrap(cacheKey, async () => {
         const { limit, offset, sortBy, sortOrder } = queryParams;
         logger.debug(`Fetching bookmarks for tag ${tagId} (DB), user ${userId} with limit: ${limit}, offset: ${offset}, sortBy: ${sortBy}, sortOrder: ${sortOrder}`);

        const where: Prisma.BookmarkWhereInput = { userId, isDeleted: false, tags: { some: { tagId } } };
        ...
        try {
            ...
            logger.info(`Found ${bookmarks.length} bookmarks (DB) (total: ${totalCount}) for tag ${tagId}, user ${userId}`);
            ...
            logger.error(`Error fetching bookmarks (DB) for tag ${tagId}, user ${userId}:`, error);
            throw new TagError('Failed to fetch bookmarks by tag', 500);
        }
     });

     if (cachedResult === null) throw new TagError('Failed to fetch bookmarks by tag', 500);
     return cachedResult;
};
