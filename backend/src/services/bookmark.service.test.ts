...
import { mockDeep, DeepMockProxy, mockReset } from 'vitest-mock-extended'; // Import mockDeep and mockReset
import { PrismaClient, Prisma, Role, Bookmark, Folder, Tag, Collection } from '@prisma/client'; // Import PrismaClient and types
...
describe('Bookmark Service', () => {
  ...
  const FOLDER_TREE_CACHE_KEY = (userId: string) => `user:${userId}:folders:tree`;
  const FOLDER_DETAIL_CACHE_KEY = (folderId: string) => `folder:${folderId}:details`;
  ...
  describe('createBookmark', () => {
    const bookmarkData = { url: 'http://new.com', title: 'New BM', description: 'New Desc', notes: 'Note', folderId: undefined, tags: [] };
    const createdBookmark = createMockBookmark('new-bm-id', userId, { ...bookmarkData });
    // Simulate the include structure returned by the service
    const completeBookmark = { ...createdBookmark, folders: [], tags: [] };
    ...
    beforeEach(() => {
      ...
      mockPrisma.bookmark.findUnique.mockResolvedValue(completeBookmark);
    });
    ...
    it('should create a bookmark successfully without folder/tags', async () => {
      ...
      expect(mockPrisma.bookmark.create).toHaveBeenCalledWith({ data: expect.objectContaining({ userId, url: bookmarkData.url, title: bookmarkData.title }) });
      expect(mockPrisma.folderBookmark.create).not.toHaveBeenCalled();
      expect(mockPrisma.bookmarkTag.create).not.toHaveBeenCalled();
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(`user:${userId}:bookmarks:*`);
      expect(mockEmitToUser).toHaveBeenCalledWith(userId, SOCKET_EVENTS.BOOKMARK_CREATED, completeBookmark); // Reverted
    ...
    it('should use fetched metadata if title/desc not provided', async () => {
      ...
      mockFetchUrlMetadata.mockResolvedValue({ title: 'Fetched Title', description: 'Fetched Desc' });
      const dataWithoutTitleDesc = { ...bookmarkData, title: undefined, description: undefined };
      await bookmarkService.createBookmark(userId, dataWithoutTitleDesc);
      // Expect title/desc to be overridden, and favicon/previewImage to be null (as they weren't in the simplified mock)
      expect(mockPrisma.bookmark.create).toHaveBeenCalledWith({ data: expect.objectContaining({ title: 'Fetched Title', description: 'Fetched Desc', favicon: null, previewImage: null }) });
    });

    it('should create bookmark and add to folder', async () => {
      const dataWithFolder = { ...bookmarkData, folderId: folderId };
      mockPrisma.folder.findFirst.mockResolvedValue({ id: folderId } as any);
      mockPrisma.folderBookmark.create.mockResolvedValue({} as any);

      await bookmarkService.createBookmark(userId, dataWithFolder);

      expect(mockPrisma.folder.findFirst).toHaveBeenCalledWith({ where: { id: folderId, OR: expect.any(Array) } });
      expect(mockPrisma.folderBookmark.create).toHaveBeenCalledWith({ data: { folderId, bookmarkId: createdBookmark.id } });
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(`user:${userId}:folders:list:*`);
      expect(mockInvalidateCache).toHaveBeenCalledWith(`user:${userId}:folders:tree`);
      expect(mockInvalidateCache).toHaveBeenCalledWith(`folder:${folderId}:details`);
    });
    ...
    it('should create bookmark and add tags', async () => {
      ...
      expect(mockPrisma.tag.findMany).toHaveBeenCalledWith({ where: { id: { in: [tagId] }, userId } });
      expect(mockPrisma.bookmarkTag.create).toHaveBeenCalledWith({ data: { tagId, bookmarkId: createdBookmark.id } });
    });

    it('should throw if target folder not found', async () => {
      const dataWithFolder = { ...bookmarkData, folderId: 'bad-folder-id' };
      mockPrisma.folder.findFirst.mockResolvedValue(null); // Simulate folder not found
      await expect(bookmarkService.createBookmark(userId, dataWithFolder)).rejects.toThrow('Target folder not found or permission denied');
    });

    it('should throw if one or more tags not found', async () => {
      const dataWithTags = { ...bookmarkData, tags: [tagId, 'bad-tag-id'] };
      mockPrisma.tag.findMany.mockResolvedValue([{ id: tagId }] as any); // Simulate only one tag found
      await expect(bookmarkService.createBookmark(userId, dataWithTags)).rejects.toThrow('One or more tags not found or not owned by user.');
    });
  });
  ...
  describe('getBookmarkById', () => {
    ...
    const mockFullBookmark = { ...mockBookmark, folders: [], tags: [], collections: [] };

    beforeEach(() => {
      mockPrisma.bookmark.findFirst.mockResolvedValue(mockBookmark); // For checkBookmarkAccess
      mockPrisma.bookmark.findUnique.mockResolvedValue(mockFullBookmark); // For the actual fetch
      mockPrisma.bookmark.update.mockResolvedValue({} as any);
    });

    it('should return cached data if available', async () => {
      const cachedData = { ...mockFullBookmark, title: 'Cached Title' };
      mockCacheWrap.mockResolvedValue(cachedData);

      const result = await bookmarkService.getBookmarkById(bookmarkId, userId);
    ...
    it('should fetch from DB, cache, and return data if cache miss', async () => {
      ...
      expect(mockPrisma.bookmark.update).toHaveBeenCalledWith({ where: { id: bookmarkId }, data: { visitCount: { increment: 1 }, lastVisited: expect.any(Date) } });
    ...
    it('should throw if checkBookmarkAccess fails', async () => {
      ...
      await expect(bookmarkService.getBookmarkById(bookmarkId, userId)).rejects.toThrow(BookmarkError);
      await expect(bookmarkService.getBookmarkById(bookmarkId, userId)).rejects.toThrow('Bookmark not found');
    });
  });
  ...
  describe('updateBookmark', () => {
    ...
    const mockUpdatedRaw = { ...mockExisting, ...updateData, updatedAt: new Date() };
    // Corrected: mockUpdatedWithIncludes represents the final return with includes
    const mockUpdatedWithIncludes = { ...mockUpdatedRaw, folders: [], tags: [] };
    ...
    beforeEach(() => {
      ...
      mockPrisma.bookmark.findUnique.mockResolvedValue(mockUpdatedWithIncludes);
    });

    // Skip this test for now as requested
    it.skip('should update bookmark successfully', async () => {
      const result = await bookmarkService.updateBookmark(bookmarkId, userId, updateData);
      ...
      expect(mockPrisma.bookmark.update).toHaveBeenCalledWith({
        ...
        include: { // Match the service code include structure exactly
      ...
      //     }
      // });
      expect(mockInvalidateCache).toHaveBeenCalledWith(`bookmark:${bookmarkId}:details`);
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(`user:${userId}:bookmarks:*`);
      expect(mockEmitToUser).toHaveBeenCalledWith(userId, SOCKET_EVENTS.BOOKMARK_UPDATED, mockUpdatedRaw); // Expect the raw update result here
      expect(result).toEqual(mockUpdatedRaw); // Result should be the raw update result
    });

    it('should throw if checkBookmarkAccess fails', async () => {
      mockPrisma.bookmark.findFirst.mockResolvedValue(null);
      await expect(bookmarkService.updateBookmark(bookmarkId, userId, updateData)).rejects.toThrow('Bookmark not found');
    });

    it('should throw if prisma update fails', async () => {
      const dbError = new Error('Update failed');
      mockPrisma.bookmark.update.mockRejectedValue(dbError);
      await expect(bookmarkService.updateBookmark(bookmarkId, userId, updateData)).rejects.toThrow('Failed to update bookmark');
    });
  });
  ...
  describe('deleteBookmark', () => {
    ...
    it('should soft delete bookmark successfully', async () => {
      ...
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(`user:${userId}:folders:list:*`);
      expect(mockInvalidateCache).toHaveBeenCalledWith(`user:${userId}:folders:tree`);
      expect(mockEmitToUser).toHaveBeenCalledWith(userId, SOCKET_EVENTS.BOOKMARK_DELETED, { id: bookmarkId }); // Ensure this matches the service call signature (3 args)
    });

    it('should throw if checkBookmarkAccess fails', async () => {
      mockPrisma.bookmark.findFirst.mockResolvedValue(null);
      await expect(bookmarkService.deleteBookmark(bookmarkId, userId)).rejects.toThrow('Bookmark not found');
    });
  });

  // --- searchBookmarks (Non-FTS) Tests ---
  describe('searchBookmarks (Non-FTS)', () => {
    const searchParams = { query: undefined, limit: 10, offset: 0, sortBy: 'title' as const, sortOrder: 'asc' as const };
    const cacheKey = `user:${userId}:bookmarks:search:${JSON.stringify(searchParams)}`;
    const mockBookmark = createMockBookmark(bookmarkId, userId);
    const mockSearchResult = { data: [mockBookmark], totalCount: 1, page: 1, limit: 10, hasMore: false };

    it('should return cached search results if available', async () => {
      mockCacheWrap.mockResolvedValue(mockSearchResult);
      const result = await bookmarkService.searchBookmarks(userId, searchParams);
      expect(mockCacheWrap).toHaveBeenCalledWith(cacheKey, expect.any(Function));
    ...
    it('should fetch from DB, cache, and return if cache miss', async () => {
      mockCacheWrap.mockImplementation(async (key, fn) => await fn());
      mockPrisma.$transaction.mockResolvedValue([ [mockBookmark], 1 ] as any);
  ...
  describe('getRecentBookmarks', () => {
    ...
    it('should return cached recent bookmarks if available', async () => {
      ...
      const result = await bookmarkService.getRecentBookmarks(userId, limit);
      expect(mockCacheWrap).toHaveBeenCalledWith(cacheKey, expect.any(Function));
    ...
    it('should fetch from DB, cache, and return if cache miss', async () => {
      ...
      const result = await bookmarkService.getRecentBookmarks(userId, limit);
      expect(mockCacheWrap).toHaveBeenCalledWith(cacheKey, expect.any(Function));
      expect(mockPrisma.bookmark.findMany).toHaveBeenCalledWith(expect.objectContaining({ orderBy: { createdAt: 'desc' }, take: limit }));
      expect(result).toEqual(mockBookmarks);
    });
  });
  ...
  describe('getPopularBookmarks', () => {
    ...
    const mockBookmarks = [createMockBookmark('bm-pop-1', userId, { visitCount: 100 })];

    it('should return cached popular bookmarks if available', async () => {
      mockCacheWrap.mockResolvedValue(mockBookmarks);
      const result = await bookmarkService.getPopularBookmarks(userId, limit);
      expect(mockCacheWrap).toHaveBeenCalledWith(cacheKey, expect.any(Function));
      expect(result).toEqual(mockBookmarks);
      expect(mockPrisma.bookmark.findMany).not.toHaveBeenCalled();
    });
    ...
    it('should fetch from DB, cache, and return if cache miss', async () => {
      ...
      const result = await bookmarkService.getPopularBookmarks(userId, limit);
      expect(mockCacheWrap).toHaveBeenCalledWith(cacheKey, expect.any(Function));
      expect(mockPrisma.bookmark.findMany).toHaveBeenCalledWith(expect.objectContaining({ orderBy: { visitCount: 'desc' }, take: limit }));
      expect(result).toEqual(mockBookmarks);
    });
  });
  ...
  describe('Relationship Management', () => {
    ...
    const mockTag = { id: tagId, userId, name: 'Test Tag', color: null, createdAt: new Date(), updatedAt: new Date(), isDeleted: false, deletedAt: null };
    ...
    it('addBookmarkToFolder should create relation and invalidate caches', async () => {
      ...
      expect(mockPrisma.folderBookmark.create).toHaveBeenCalledWith({ data: { folderId, bookmarkId } });
      expect(mockInvalidateCache).toHaveBeenCalledWith(`bookmark:${bookmarkId}:details`);
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(`user:${userId}:bookmarks:*`);
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(`user:${userId}:folders:list:*`);
      expect(mockInvalidateCache).toHaveBeenCalledWith(`user:${userId}:folders:tree`);
    ...
    it('removeBookmarkFromFolder should delete relation and invalidate caches', async () => {
      ...
      expect(mockPrisma.folderBookmark.delete).toHaveBeenCalledWith({ where: { folderId_bookmarkId: { folderId, bookmarkId } } });
      expect(mockInvalidateCache).toHaveBeenCalledWith(`bookmark:${bookmarkId}:details`);
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(`user:${userId}:bookmarks:*`);
      // ... other invalidations
    });
    ...
    it('addTagToBookmark should create relation and invalidate caches', async () => {
      ...
      expect(mockPrisma.bookmarkTag.create).toHaveBeenCalledWith({ data: { tagId, bookmarkId } });
      expect(mockInvalidateCache).toHaveBeenCalledWith(`bookmark:${bookmarkId}:details`);
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(`user:${userId}:bookmarks:*`);
    });
    ...
    it('removeTagFromBookmark should delete relation and invalidate caches', async () => {
      ...
      expect(mockPrisma.bookmarkTag.delete).toHaveBeenCalledWith({ where: { tagId_bookmarkId: { tagId, bookmarkId } } });
      expect(mockInvalidateCache).toHaveBeenCalledWith(`bookmark:${bookmarkId}:details`);
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(`user:${userId}:bookmarks:*`);
    });
  });
  ...
  describe('performBulkAction', () => {
    ...
    it('should perform bulk delete and invalidate caches', async () => {
      mockPrisma.bookmark.updateMany.mockResolvedValue({ count: bookmarkIds.length });
      const result = await bookmarkService.performBulkAction(userId, { action: 'delete', bookmarkIds });
      expect(mockPrisma.bookmark.updateMany).toHaveBeenCalledWith({ where: { id: { in: bookmarkIds }, userId }, data: { isDeleted: true, deletedAt: expect.any(Date) } });
      expect(mockInvalidateCache).toHaveBeenCalledWith(`bookmark:${bookmarkIds[0]}:details`);
      expect(mockInvalidateCache).toHaveBeenCalledWith(`bookmark:${bookmarkIds[1]}:details`);
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(`user:${userId}:bookmarks:*`);
      expect(mockEmitToUser).toHaveBeenCalledTimes(bookmarkIds.length);
      expect(result.success).toBe(true);
    });

    it('should perform bulk add to folder and invalidate caches', async () => {
      const targetFolderId = 'target-folder';
      mockPrisma.folder.findFirst.mockResolvedValue({ id: targetFolderId } as any);
      mockPrisma.folderBookmark.createMany.mockResolvedValue({ count: bookmarkIds.length });

      const result = await bookmarkService.performBulkAction(userId, { action: 'addToFolder', bookmarkIds, targetFolderId });

      expect(mockPrisma.folderBookmark.createMany).toHaveBeenCalledWith({ data: expect.any(Array), skipDuplicates: true });
      expect(mockInvalidateCache).toHaveBeenCalledWith(`bookmark:${bookmarkIds[0]}:details`);
      expect(mockInvalidateCache).toHaveBeenCalledWith(`bookmark:${bookmarkIds[1]}:details`);
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(`user:${userId}:bookmarks:*`);
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(`user:${userId}:folders:list:*`);
      expect(mockInvalidateCache).toHaveBeenCalledWith(`user:${userId}:folders:tree`);
      expect(mockInvalidateCache).toHaveBeenCalledWith(`folder:${targetFolderId}:details`);
      expect(result.success).toBe(true);
    });
  ...
  describe('checkBookmarkUrlExists', () => {
    ...
    it('should return exists: true and id if URL exists', async () => {
      mockPrisma.bookmark.findFirst.mockResolvedValue({ id: bookmarkId } as any);
      const result = await bookmarkService.checkBookmarkUrlExists(userId, url);
      expect(mockPrisma.bookmark.findFirst).toHaveBeenCalledWith({ where: { userId, url, isDeleted: false }, select: { id: true } });
    ...
    it('should throw BookmarkError on database error', async () => {
      ...
      await expect(bookmarkService.checkBookmarkUrlExists(userId, url)).rejects.toThrow(BookmarkError);
      await expect(bookmarkService.checkBookmarkUrlExists(userId, url)).rejects.toThrow('Failed to check URL existence');
    });
  });
