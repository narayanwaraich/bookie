...
import { TagError, TAG_LIST_CACHE_KEY, TAG_DETAIL_CACHE_KEY, TAG_BOOKMARKS_CACHE_KEY, USER_TAG_LIST_PATTERN } from './tag.service'; // Import necessary constants/types
...
describe('Tag Service', () => {
  ...
  const createMockTag = (id: string, userId: string, overrides = {}): Tag => ({
    id, userId,
    name: `Tag ${id}`,
    color: '#808080',
  ...
  describe('createTag', () => {
    ...
    it('should create a tag successfully', async () => {
      ...
      expect(mockPrisma.tag.findFirst).toHaveBeenCalledWith({ where: { name: tagData.name, userId, isDeleted: false } });
      expect(mockPrisma.tag.create).toHaveBeenCalledWith({ data: { ...tagData, userId } });
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(USER_TAG_LIST_PATTERN(userId));
      expect(mockEmitToUser).toHaveBeenCalledWith(expect.any(Object), userId, SOCKET_EVENTS.TAG_CREATED, createdTag); // Added expect.any(Object) for ioInstance
      expect(result).toEqual(createdTag);
    });

    it('should create a tag with default color if not provided', async () => {
      const dataWithoutColor = { name: 'Tag No Color' };
      const createdTagWithDefaultColor = createMockTag('no-color-id', userId, { ...dataWithoutColor, color: '#808080' });
      mockPrisma.tag.findFirst.mockResolvedValue(null);
      mockPrisma.tag.create.mockResolvedValue(createdTagWithDefaultColor);
      await tagService.createTag(userId, dataWithoutColor);
      expect(mockPrisma.tag.create).toHaveBeenCalledWith({ data: { name: dataWithoutColor.name, color: '#808080', userId } });
    });
    ...
    it('should throw TagError if tag name already exists', async () => {
      ...
      await expect(tagService.createTag(userId, tagData)).rejects.toThrow(TagError);
      await expect(tagService.createTag(userId, tagData)).rejects.toThrow('A tag with this name already exists');
    });
  });
  ...
  describe('getAllUserTags', () => {
    ...
    const mockTagWithCount = { ...mockDbTag, _count: { bookmarks: 3 } };
    const mockServiceResult = { data: [{ ...mockDbTag, bookmarkCount: 3 }], totalCount: 1, page: 1, limit: 10, hasMore: false };
    ...
    it('should fetch from DB, cache, and return data if cache miss', async () => {
      mockCacheWrap.mockImplementation(async (key, fn) => await fn());
      mockPrisma.$transaction.mockResolvedValue([ [mockTagWithCount], 1 ] as any);
    ...
    it('should throw TagError if cacheWrap returns null', async () => {
      ...
      await expect(tagService.getAllUserTags(userId, queryParams)).rejects.toThrow(TagError);
      await expect(tagService.getAllUserTags(userId, queryParams)).rejects.toThrow('Failed to fetch tags');
    });
  });
  ...
  describe('getTagById', () => {
    ...
    const mockDbTag = createMockTag(tagId, userId);
    const mockTagWithCount = { ...mockDbTag, _count: { bookmarks: 2 } };
    ...
    it('should throw TagError if cached data is not owned by user', async () => {
      const cachedDataWrongUser = { ...mockServiceResult, userId: 'other-user' };
      mockCacheWrap.mockResolvedValue(cachedDataWrongUser);
      await expect(tagService.getTagById(tagId, userId)).rejects.toThrow(TagError);
      await expect(tagService.getTagById(tagId, userId)).rejects.toThrow('Tag not found');
    });
    ...
    it('should fetch from DB, cache, and return data if cache miss', async () => {
      ...
      const result = await tagService.getTagById(tagId, userId);
      expect(mockCacheWrap).toHaveBeenCalledWith(cacheKey, expect.any(Function));
      expect(mockPrisma.tag.findFirst).toHaveBeenCalledWith({ where: { id: tagId, userId, isDeleted: false }, include: expect.any(Object) });
      expect(result).toEqual(mockServiceResult);
    });
  ...
  describe('updateTag', () => {
    ...
    const mockUpdatedTag = { ...mockExistingTag, ...updateData, updatedAt: new Date() };
    ...
    it('should update tag successfully', async () => {
      ...
      expect(mockPrisma.tag.findFirst).toHaveBeenCalledWith({ where: { id: tagId, userId, isDeleted: false } });
      expect(mockPrisma.tag.findFirst).toHaveBeenCalledWith({ where: { name: updateData.name, userId, id: { not: tagId }, isDeleted: false } });
      expect(mockPrisma.tag.update).toHaveBeenCalledWith({ where: { id: tagId }, data: updateData });
      expect(mockInvalidateCache).toHaveBeenCalledWith(TAG_DETAIL_CACHE_KEY(tagId));
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(USER_TAG_LIST_PATTERN(userId));
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(`user:${userId}:bookmarks:*`);
      expect(mockEmitToUser).toHaveBeenCalledWith(expect.any(Object), userId, SOCKET_EVENTS.TAG_UPDATED, mockUpdatedTag); // Added expect.any(Object) for ioInstance
    ...
    it('should throw TagError if tag not found', async () => {
      ...
      await expect(tagService.updateTag(tagId, userId, updateData)).rejects.toThrow(TagError);
      await expect(tagService.updateTag(tagId, userId, updateData)).rejects.toThrow('Tag not found');
    ...
    it('should throw TagError if name already exists', async () => {
      ...
      mockPrisma.tag.findFirst.mockResolvedValue(createMockTag('other-tag', userId)); // Duplicate found
      await expect(tagService.updateTag(tagId, userId, updateData)).rejects.toThrow(TagError);
      await expect(tagService.updateTag(tagId, userId, updateData)).rejects.toThrow('Another tag with this name already exists');
    });
  });
  ...
  describe('deleteTag', () => {
    ...
    it('should soft delete tag successfully', async () => {
      ...
      expect(mockPrisma.tag.findFirst).toHaveBeenCalledWith({ where: { id: tagId, userId, isDeleted: false } });
      expect(mockPrisma.tag.update).toHaveBeenCalledWith({ where: { id: tagId }, data: { isDeleted: true, deletedAt: expect.any(Date) } });
      expect(mockInvalidateCache).toHaveBeenCalledWith(TAG_DETAIL_CACHE_KEY(tagId));
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(USER_TAG_LIST_PATTERN(userId));
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(TAG_BOOKMARKS_CACHE_KEY(tagId, '*'));
      expect(mockInvalidateCachePattern).toHaveBeenCalledWith(`user:${userId}:bookmarks:*`);
      expect(mockEmitToUser).toHaveBeenCalledWith(expect.any(Object), userId, SOCKET_EVENTS.TAG_DELETED, { id: tagId }); // Added expect.any(Object) for ioInstance
    });

    it('should throw TagError if tag not found', async () => {
      mockPrisma.tag.findFirst.mockResolvedValue(null);
      await expect(tagService.deleteTag(tagId, userId)).rejects.toThrow(TagError);
      await expect(tagService.deleteTag(tagId, userId)).rejects.toThrow('Tag not found');
    });
  });

  // --- getBookmarksByTag Tests ---
  describe('getBookmarksByTag', () => {
    const queryParams = { limit: 10, offset: 0, sortBy: 'createdAt' as const, sortOrder: 'desc' as const };
    ...
    const mockServiceResult = { data: [mockBookmark], totalCount: 1, page: 1, limit: 10, hasMore: false };
    ...
    it('should throw TagError if tag not found', async () => {
        ...
        await expect(tagService.getBookmarksByTag(tagId, userId, queryParams)).rejects.toThrow(TagError);
        await expect(tagService.getBookmarksByTag(tagId, userId, queryParams)).rejects.toThrow('Tag not found');
    });

    it('should throw TagError if cacheWrap returns null', async () => {
        mockCacheWrap.mockResolvedValue(null);
        await expect(tagService.getBookmarksByTag(tagId, userId, queryParams)).rejects.toThrow(TagError);
        await expect(tagService.getBookmarksByTag(tagId, userId, queryParams)).rejects.toThrow('Failed to fetch bookmarks by tag');
    });
  });
