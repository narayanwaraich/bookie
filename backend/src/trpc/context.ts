 ...
 */
const getTokenFromHeader = (req: CreateExpressContextOptions['req']): string | null => {
...
export const createContext = async ({
  ...
  if (token) {
    try {
      ...
      if (dbUser) {
        user = dbUser;
        logger.debug(`[tRPC Context]: User ${user.id} authenticated via token.`);
      } else {
         logger.warn(`[tRPC Context]: User ${decoded.id} from token not found or inactive.`);
      }
    } catch (error) {
      // Ignore token errors (expired, invalid) - user remains null
      if (error instanceof jwt.JsonWebTokenError || error instanceof jwt.TokenExpiredError) {
         logger.warn(`[tRPC Context]: Invalid or expired token received.`);
      } else {
