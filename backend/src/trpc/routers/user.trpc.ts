  ...
  getProfile: protectedProcedure
    .query(async ({ ctx }) => {
      ...
      try {
        ...
        if (error instanceof UserServiceError && error.statusCode === 404) throw new TRPCError({ code: 'NOT_FOUND', message: error.message, cause: error });
        throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'Failed to fetch profile', cause: error });
      }
    }),
  ...
  updateProfile: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        if (error instanceof UserServiceError && (error.statusCode === 400 || error.statusCode === 409 || error.statusCode === 404)) {
             throw new TRPCError({ code: 'BAD_REQUEST', message: error.message, cause: error });
        }
        throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'Failed to update profile', cause: error });
      }
    }),
    ...
    .mutation(async ({ ctx, input }) => {
        ...
        try {
            return await userService.changeUserPassword(userId, input);
             ...
             if (error instanceof UserServiceError && (error.statusCode === 400 || error.statusCode === 404)) {
                 throw new TRPCError({ code: 'BAD_REQUEST', message: error.message, cause: error });
             }
             throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'Failed to change password', cause: error });
        }
    }),
  ...
  deleteAccount: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
        ...
        try {
            ...
            return await userService.deleteUserAccount(userId, input.password);
