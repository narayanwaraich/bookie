...
describe('User TRPC Router', () => {
  ...
  describe('getProfile', () => {
    it('should call userService.getUserProfile and return the profile', async () => {
        const serviceResult = { id: userId, name: 'TRPC User', email: 'test@trpc.com', /* ... other profile fields */ };
        mockUserService.getUserProfile.mockResolvedValue(serviceResult as any);
  ...
  describe('updateProfile', () => {
     it('should call userService.updateUserProfile and return the updated profile', async () => {
        const input: inferProcedureInput<typeof appRouter.user.updateProfile> = { name: 'Updated TRPC User' }; 
        const serviceResult = { id: userId, name: input.name, email: 'test@trpc.com', /* ... */ };
        mockUserService.updateUserProfile.mockResolvedValue(serviceResult as any);
        const caller = await createCaller();

        const result = await caller.user.updateProfile(input); 

        expect(userService.updateUserProfile).toHaveBeenCalledWith(userId, input);
        expect(result).toEqual(serviceResult); 
     });
      // TODO: Add error test
  });
  ...
  describe('changePassword', () => {
     it('should call userService.changeUserPassword', async () => {
        const input: inferProcedureInput<typeof appRouter.user.changePassword> = { currentPassword: 'old', newPassword: 'new' }; 
        ...
        expect(userService.changeUserPassword).toHaveBeenCalledWith(userId, input);
  ...
  describe('deleteAccount', () => {
     it('should call userService.deleteUserAccount', async () => {
        const input: inferProcedureInput<typeof appRouter.user.deleteAccount> = { password: 'password' }; 
        ...
        expect(userService.deleteUserAccount).toHaveBeenCalledWith(userId, input.password);
         // Explicitly type the expected object
        const expectedResult: { success: boolean } = { success: true };
        expect(result).toEqual(expectedResult); 
     });
