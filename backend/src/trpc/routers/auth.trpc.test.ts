...
import * as authService from '../../services/auth.service'; // Import the service to mock
...
describe('Auth TRPC Router', () => {
  ...
  beforeEach(() => {
    ...
    mockCreateContext.mockResolvedValue({ ...mockContextBase, user: null } as any); 
  });

  // Helper to create a caller instance
  const createCaller = async (user: { id: string, email: string, username: string } | null = null) => {
  ...
  describe('register', () => {
    it('should call authService.registerUser and return tokens/user', async () => {
      // Arrange
      const registerInput: inferProcedureInput<typeof appRouter.auth.register> = { 
          email: 'trpc@test.com', username: 'trpctest', password: 'password', name: 'TRPC User' 
      };
      const serviceResult = {
        token: 'trpc-access-token',
        refreshToken: 'trpc-refresh-token',
        // Ensure name matches expected type (string | null)
        user: { id: 'trpc-user-1', email: registerInput.email, username: registerInput.username, name: registerInput.name || null, isVerified: false } 
      };
      mockAuthService.registerUser.mockResolvedValue(serviceResult as any); // Cast if needed due to complex type
      ...
      expect(result).toEqual({ success: true, accessToken: serviceResult.token, user: serviceResult.user }); 
    });
  ...
  describe('login', () => {
     it('should call authService.loginUser and return tokens/user', async () => {
      // Arrange
      const loginInput: inferProcedureInput<typeof appRouter.auth.login> = { 
          email: 'trpc@test.com', password: 'password' 
      };
      const serviceResult = {
        token: 'trpc-access-token',
        refreshToken: 'trpc-refresh-token', 
        user: { id: 'trpc-user-1', email: loginInput.email, username: 'trpctest', name: 'TRPC User', isVerified: true }
      };
      mockAuthService.loginUser.mockResolvedValue(serviceResult as any); 
      const caller = await createCaller(); 
   ...
   describe('logout', () => {
       it('should call authService.logoutUser', async () => {
           // Arrange
           const mockUser = { id: 'logged-in-user', email: 'in@test.com', username: 'loggedin' };
           ...
           // Assert
           expect(authService.logoutUser).toHaveBeenCalledWith(mockUser.id);
   ...
   describe('refreshToken', () => {
       it('should call authService.refreshTokenService and return new token/user', async () => {
           ...
           const refreshedUser = { id: 'refreshed-user', email: 'refresh@test.com', username: 'refresher', name: 'Refreshed User' };
           
           // Mock context to include the old refresh token in cookies
           const contextWithToken = await createContext({ 
               req: { headers: {}, cookies: { refreshToken: oldRefreshToken } }, 
               res: { cookie: vi.fn(), clearCookie: vi.fn() } 
           } as any);
           ...
           mockAuthService.refreshTokenService.mockResolvedValue(serviceResult);
           // Mock the subsequent getAuthenticatedUser call made within the procedure
           mockAuthService.getAuthenticatedUser.mockResolvedValue(refreshedUser as any); 
           ...
           expect(result).toEqual({ success: true, accessToken: serviceResult.accessToken, user: refreshedUser });
       });
   ...
   describe('checkAuth', () => {
       it('should call authService.getAuthenticatedUser and return user data', async () => {
           ...
           mockAuthService.getAuthenticatedUser.mockResolvedValue(serviceResult as any);
           const caller = await createCaller(mockUser); 
           ...
           // Assert
           expect(authService.getAuthenticatedUser).toHaveBeenCalledWith(mockUser.id);
