...
import { cacheWrap, invalidateCache, invalidateCachePattern } from './cache';
...
describe('Cache Utils', () => {
  ...
  describe('cacheWrap', () => {
    ...
    it('should fetch from DB, cache, and return data if cache miss', async () => {
      ...
      expect(mockRedisSet).toHaveBeenCalledWith(testKey, JSON.stringify(dbValue), 'EX', testTtl);
    ...
    it('should handle Redis get error and fetch from DB', async () => {
      ...
      // Assert
      expect(mockRedisGet).toHaveBeenCalledWith(testKey);
      expect(mockLogger.error).toHaveBeenCalledWith(`Redis GET error for key ${testKey}:`, redisError);
      expect(dbFetchFn).toHaveBeenCalledTimes(1);
      expect(mockRedisSet).toHaveBeenCalledWith(testKey, JSON.stringify(dbValue), 'EX', testTtl);
      expect(result).toEqual(dbValue);
    });
     ...
     it('should handle Redis set error after fetching from DB', async () => {
      ...
      expect(dbFetchFn).toHaveBeenCalledTimes(1);
      expect(mockRedisSet).toHaveBeenCalledWith(testKey, JSON.stringify(dbValue), 'EX', testTtl);
      expect(mockLogger.error).toHaveBeenCalledWith(`Redis SET error for key ${testKey}:`, redisSetError);
     ...
     it('should handle DB fetch error', async () => {
      ...
      // Act & Assert
      await expect(cacheWrap(testKey, dbFetchFn, testTtl)).rejects.toThrow(dbError);
      expect(mockRedisGet).toHaveBeenCalledWith(testKey);
      expect(dbFetchFn).toHaveBeenCalledTimes(1);
      expect(mockRedisSet).not.toHaveBeenCalled();
      expect(mockLogger.error).toHaveBeenCalledWith(`Error executing dbFetchFn for key ${testKey}:`, dbError);
    });
  });
  ...
  describe('invalidateCache', () => {
    it('should call redis.del with the provided key', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(`Cache invalidated for key: ${testKey}`);
    });
    ...
    it('should handle Redis del error', async () => {
      ...
      // Assert
      expect(mockRedisDel).toHaveBeenCalledWith(testKey);
      expect(mockLogger.error).toHaveBeenCalledWith(`Redis DEL error for key ${testKey}:`, redisError);
    });
  });
  ...
  describe('invalidateCachePattern', () => {
    it('should scan for keys and delete them using pipeline', async () => {
      ...
      expect(mockRedisScan).toHaveBeenCalledWith(0, 'MATCH', pattern, 'COUNT', 100);
      expect(mockRedisScan).toHaveBeenCalledWith('10', 'MATCH', pattern, 'COUNT', 100);
      expect(mockRedisPipeline).toHaveBeenCalledTimes(1); // Pipeline called once
      expect(pipelineDelMock).toHaveBeenCalledTimes(keysToDelete.length);
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(`Invalidating cache for pattern: ${pattern}. Found ${keysToDelete.length} keys.`);
    });
    ...
    it('should handle empty scan results', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(`Invalidating cache for pattern: ${pattern}. Found 0 keys.`);
    });
    ...
    it('should handle Redis scan error', async () => {
      ...
      // Assert
      expect(mockRedisScan).toHaveBeenCalledWith(0, 'MATCH', pattern, 'COUNT', 100);
      expect(mockLogger.error).toHaveBeenCalledWith(`Redis SCAN error for pattern ${pattern}:`, redisError);
     ...
     it('should handle Redis pipeline exec error', async () => {
      ...
      const pipelineDelMock = vi.fn().mockReturnThis();
      const pipelineExecMock = vi.fn().mockRejectedValue(pipelineError);
      mockRedisPipeline.mockReturnValue({
          del: pipelineDelMock,
          exec: pipelineExecMock,
      } as any);
      ...
      expect(pipelineDelMock).toHaveBeenCalledWith(keysToDelete[0]);
      expect(pipelineExecMock).toHaveBeenCalledTimes(1);
      expect(mockLogger.error).toHaveBeenCalledWith(`Redis pipeline EXEC error for pattern ${pattern}:`, pipelineError);
    });
  });
