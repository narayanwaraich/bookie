    ...
    try {
        ...
        if (cachedData) {
            ...
            try {
                ...
                logger.error(`[Cache]: Failed to parse cached data for key "${key}". Fetching fresh data.`, parseError);
        ...
        if (freshData !== null && freshData !== undefined) {
            try {
                ...
                logger.debug(`[Cache]: SET key "${key}" with TTL ${ttlSeconds}s`);
            } catch (cacheSetError) {
                // Log error but don't fail the overall operation if caching fails
                logger.error(`[Cache]: Failed to SET key "${key}".`, cacheSetError);
            }
        }
        ...
        logger.error(`[CacheWrap]: Error during cache check or function execution for key "${key}":`, error);
...
export const invalidateCache = async (keys: string | string[]): Promise<void> => {
    ...
    try {
        ...
        logger.info(`[Cache]: Invalidated ${count} key(s): ${keysToDelete.join(', ')}`);
    } catch (error) {
        logger.error(`[Cache]: Failed to invalidate keys: ${keysToDelete.join(', ')}`, error);
...
export const invalidateCachePattern = async (pattern: string): Promise<void> => {
    logger.warn(`[Cache]: Starting pattern invalidation for pattern: "${pattern}". This can be slow.`);
    let cursor = '0';
    let keysFound = 0;
    try {
        do {
            ...
            if (keys.length > 0) {
                ...
                logger.debug(`[Cache]: Unlinked ${keys.length} keys matching pattern "${pattern}".`);
            }
        } while (cursor !== '0');
        logger.info(`[Cache]: Pattern invalidation finished for "${pattern}". Found and unlinked ${keysFound} keys.`);
    } catch (error) {
         logger.error(`[Cache]: Error during pattern invalidation for "${pattern}":`, error);
    }
};
