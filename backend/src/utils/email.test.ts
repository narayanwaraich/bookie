...
describe('Email Utility', () => {
  ...
  it('should log error if sending email fails', async () => {
     // Arrange
    const options: EmailOptions = { to: 'fail@example.com', subject: 'Fail Test', text: '...', html: '<p>...</p>' }; // Added html to satisfy type
    ...
    expect(mockLoggerError).toHaveBeenCalledWith('Error sending email:', sendError);
  });
  
   it('should handle missing optional fields (text/html)', async () => {
     // Arrange
     // Provide an empty string for text to satisfy the type
    const options: EmailOptions = { to: 'minimal@example.com', subject: 'Minimal Test', html: '<p>HTML only</p>', text: '' }; 
    mockSendMail.mockResolvedValue({ messageId: 'minimal-message-id' });
