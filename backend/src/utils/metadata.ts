...
export const fetchUrlMetadata = async (url: string): Promise<{
    ...
    try {
        logger.debug(`Fetching metadata for URL: ${url} using html-metadata-parser`);
        ...
        const description = metadata.og?.description || metadata.meta?.description;
        ...
        if (favicon && !favicon.startsWith('http')) {
            try {
                ...
                logger.warn(`Could not construct absolute favicon URL for ${favicon} relative to ${url}`);
        ...
        if (error.name === 'AbortError') {
             ...
             logger.error(`Error fetching or parsing metadata for ${url}:`, error);
        }
