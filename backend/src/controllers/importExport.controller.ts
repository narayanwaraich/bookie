...
import { ImportOptionsInput, ExportOptionsInput, exportOptionsSchema } from '../models/schemas';
 ...
 *               $ref: '#/components/schemas/ErrorResponse'
 */
export const importBookmarks = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    const file = req.file;
    ...
    try {
        logger.info(`[Import Controller] Processing uploaded file: ${file.originalname}, mimetype: ${file.mimetype}, path: ${filePath}`);
        if (file.mimetype === 'application/json' || file.mimetype === 'application/x-chrome-bookmarks') {
            format = 'json';
            ...
            importData = fs.readFileSync(filePath, 'utf-8');
             ...
             logger.error(`[Import Controller] Unsupported file type passed filter: ${file.mimetype}`);
             throw new ImportExportError('Unsupported file type.', 400);
        }

        const options: ImportOptionsInput = { format, folderId };
        
        const result = await importExportService.importBookmarks(userId, importData, options);
        logger.info(`[Import Controller] Import successful for user ${userId}. Stats: ${JSON.stringify(result.stats)}`);

        fs.unlink(filePath, (err) => {
            if (err) logger.error(`[Import Controller]: Failed to delete uploaded file ${filePath}:`, err);
            else logger.info(`[Import Controller]: Deleted uploaded file ${filePath}`);
        });

        res.status(200).json(result);

    } catch (error) {
        logger.error('[Import Controller]: Error importing bookmarks:', error);
        if (filePath) { 
            fs.unlink(filePath, (err) => {
                 if (err) logger.error(`[Import Controller]: Failed to delete uploaded file ${filePath} after error:`, err);
                 else logger.info(`[Import Controller]: Deleted uploaded file ${filePath} after error.`);
            });
        }
        next(error); 
    }
 ...
 *               $ref: '#/components/schemas/ErrorResponse'
 */
export const exportBookmarks = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    const userId = req.user!.id;
    try {
        ...
        logger.info(`[Export Controller] User ${userId} requested export with options: ${JSON.stringify(options)}`);
        ...
        if (!result) {
            ...
            logger.error(`[Export Controller] Export service returned undefined result for user ${userId}`);
            throw new ImportExportError('Export failed due to an unexpected server issue.', 500);
        }
        ...
        if (result.filename) {
            ...
            logger.info(`[Export Controller] Sending file ${result.filename} with type ${result.contentType}`);
        } else {
             logger.info(`[Export Controller] Sending data with type ${result.contentType}`);
        }
        res.status(200).send(result.data);

    } catch (error) {
         if (error instanceof z.ZodError) { // Check if error is a ZodError
            logger.error('Export query validation error:', error.errors);
         } else {
            logger.error(`[Export Controller] Error exporting bookmarks for user ${userId}:`, error);
         }
