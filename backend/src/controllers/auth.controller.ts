...
export const register = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { token, refreshToken, user } = await authService.registerUser(userData); 
...
export const login = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { token: accessToken, refreshToken, user } = await authService.loginUser(credentials); 
...
export const refreshToken = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const decodedPayload = decode(accessToken) as { sub?: string; id?: string } | null;
    const userIdFromToken = decodedPayload?.id || decodedPayload?.sub;
    if (!userIdFromToken) {
        throw new AuthError('Invalid access token payload after refresh', 500);
    }
    const user = await authService.getAuthenticatedUser(userIdFromToken);
...
export const checkAuth = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
        ...
        if (!req.user) {
            ...
            throw new AuthError('User not found after authentication check', 401);
        }
