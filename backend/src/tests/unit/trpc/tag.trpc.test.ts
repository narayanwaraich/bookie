...
import { TagError } from '../../api/services/tag.service';
...
describe('Tag TRPC Router', () => {
  ...
  describe('create', () => {
    it('should call tagService.createTag and return the new tag', async () => {
      const input: inferProcedureInput<typeof appRouter.tags.create> =
        { name: 'TRPC Tag', color: '#aabbcc' }; // Corrected path
  ...
  describe('list', () => {
    it('should call tagService.getAllUserTags and return tags', async () => {
      ...
      const input: inferProcedureInput<typeof appRouter.tags.list> =
        {}; // Corrected path
      const serviceResult = {
        data: [],
        totalCount: 0,
        ...
        limit: 100,
        hasMore: false,
      }; // Service returns paginated
      mockTagService.getAllUserTags.mockResolvedValue(
        serviceResult as any
      );
      const caller = await createCaller();
  ...
  describe('getById', () => {
    it('should call tagService.getTagById and return the tag', async () => {
      const input: inferProcedureInput<
        typeof appRouter.tags.getById
      > = { id: tagId }; // Corrected path
      const serviceResult = {
        id: tagId,
        name: 'TRPC Get Tag',
        userId,
  ...
  describe('update', () => {
    it('should call tagService.updateTag', async () => {
      const input: inferProcedureInput<typeof appRouter.tags.update> =
        { id: tagId, name: 'Updated TRPC Tag' }; // Corrected path
  ...
  describe('delete', () => {
    it('should call tagService.deleteTag', async () => {
      const input: inferProcedureInput<typeof appRouter.tags.delete> =
        { id: tagId }; // Corrected path
  ...
  describe('getBookmarks', () => {
    it('should call tagService.getBookmarksByTag', async () => {
      const input: inferProcedureInput<
        typeof appRouter.tags.getBookmarks
      > = { tagId: tagId, limit: 10, offset: 0 }; // Corrected path
      const serviceResult = {
        data: [],
        totalCount: 0,
        page: 1,
      ...
      mockTagService.getBookmarksByTag.mockResolvedValue(
        serviceResult as any
      );
      const caller = await createCaller();
