...
import * as syncService from '../../api/services/sync.service';
import { SyncError } from '../../api/services/sync.service';
...
describe('Sync TRPC Router', () => {
  ...
  describe('sync', () => {
    it('should call syncService.syncData and return the sync result', async () => {
      ...
      mockSyncService.syncData.mockResolvedValue(
    ...
    it('should throw TRPCError if syncService throws SyncError', async () => {
      ...
      await expect(caller.sync.sync(input)).rejects.toThrowError(
        /Sync conflict/
      );
