...
const mockCheerioElementAPI = {
  ...
  each: vi.fn(function (
    ...
    (this._elements || []).forEach((el: any, index: number) =>
      callback(index, el)
    );
    return this;
  }),
...
describe('Import/Export Controller', () => {
  ...
  describe('importBookmarks', () => {
    it('should handle HTML import successfully', async () => {
      ...
      // Act
      await importExportController.importBookmarks(
    ...
    it('should handle CSV import successfully', async () => {
      ...
      const serviceResult = {
        success: true,
        stats: { total: 1, imported: 1, failed: 0 },
        message: 'Import complete',
      };
      ...
      expect(
        importExportService.importBookmarks
      ).toHaveBeenCalledWith(userId, csvContent, {
        format: 'csv',
        folderId: 'folder-xyz',
      });
      expect(statusMock).toHaveBeenCalledWith(200);
    ...
    it('should handle JSON import successfully', async () => {
      ...
      expect(
        importExportService.importBookmarks
      ).toHaveBeenCalledWith(userId, jsonObject, {
        format: 'json',
        folderId: undefined,
      });
      expect(statusMock).toHaveBeenCalledWith(200);
    ...
    it('should call next with error if service fails', async () => {
      ...
      const serviceError = new ImportExportError(
        'Import failed',
        500
      );
      mockRequest.file = mockFile;
  ...
  describe('exportBookmarks', () => {
    it('should handle CSV export successfully', async () => {
      ...
      // Act
      await importExportController.exportBookmarks(
        mockRequest as AuthenticatedRequest,
        mockResponse as Response,
        mockNext
      ); // Use correct controller name
      ...
      expect(
        importExportService.exportBookmarks
      ).toHaveBeenCalledWith(userId, exportOptions);
      expect(mockResponse.setHeader).toHaveBeenCalledWith(
        'Content-Disposition',
        `attachment; filename="${serviceResult.filename}"`
      );
      expect(mockResponse.setHeader).toHaveBeenCalledWith(
        'Content-Type',
