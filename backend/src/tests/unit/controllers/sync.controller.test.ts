...
describe('Sync Controller', () => {
  ...
  describe('handleSync', () => {
    it('should process sync data successfully and return sync result', async () => {
      ...
      vi.mocked(syncService.syncData).mockResolvedValue(
        serviceResult as any
      );

      // Act
      await syncController.handleSync(
      ...
      expect(syncService.syncData).toHaveBeenCalledWith(
        userId,
        syncInput.lastSyncTimestamp,
        syncInput.clientChanges
      );
    ...
    it('should call next with error if sync service fails', async () => {
      ...
      const serviceError = new SyncError(
    ...
    it('should call next with error if input validation fails (simulated)', async () => {
      ...
      vi.mocked(syncService.syncData).mockRejectedValue(
        validationError
      );
      ...
      expect(syncService.syncData).toHaveBeenCalledWith(
        userId,
        invalidSyncInput.lastSyncTimestamp,
        invalidSyncInput.clientChanges
      );
