...
describe('Collection Controller', () => {
  ...
  describe('createCollection', () => {
    it('should create a collection successfully and return 201', async () => {
      ...
      expect(collectionService.createCollection).toHaveBeenCalledWith(
        { ...collectionInput, userId, ownerId: userId }
      );
    ...
    it('should call next with error if service fails', async () => {
      ...
      const serviceError = new CollectionError(
        'Failed to create',
        500
      );
      mockRequest.body = collectionInput;
  ...
  describe('getUserCollections', () => {
    it('should get user collections successfully', async () => {
      ...
      // Act
      await collectionController.getUserCollections(
        mockRequest as AuthenticatedRequest,
        mockResponse as Response,
        mockNext
      );
      ...
      expect(
        collectionService.getUserCollections
      ).toHaveBeenCalledWith(userId, expectedServiceParams);
  ...
  describe('getCollectionById', () => {
    it('should get a specific collection successfully', async () => {
      ...
      ).mockResolvedValue(collectionData as any);
  ...
  describe('getPublicCollectionByLink', () => {
    it('should get a public collection successfully', async () => {
      ...
      // Act
      await collectionController.getPublicCollectionByLink(
      ...
      expect(
        collectionService.getPublicCollectionByLink
      ).toHaveBeenCalledWith(publicLink);
      expect(statusMock).toHaveBeenCalledWith(200);
      expect(jsonMock).toHaveBeenCalledWith({
        success: true,
        collection: collectionData,
  ...
  describe('addBookmarkToCollection', () => {
    it('should add bookmark to collection successfully', async () => {
      ...
      // Act
      await collectionController.addBookmarkToCollection(
        mockRequest as AuthenticatedRequest,
        mockResponse as Response,
        mockNext
      );
      ...
      expect(
        collectionService.addBookmarkToCollection
      ).toHaveBeenCalledWith(collectionId, bookmarkId, userId);
      expect(mockResponse.sendStatus).toHaveBeenCalledWith(204);
      expect(mockNext).not.toHaveBeenCalled();
    });
    // TODO: Add test for service error
  ...
  describe('removeBookmarkFromCollection', () => {
    it('should remove bookmark from collection successfully', async () => {
      ...
      // Act
      await collectionController.removeBookmarkFromCollection(
        mockRequest as AuthenticatedRequest,
        mockResponse as Response,
        mockNext
      );
