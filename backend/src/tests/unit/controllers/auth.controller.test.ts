...
describe('Auth Controller', () => {
  ...
  describe('refreshToken', () => {
    it('should refresh token successfully if valid refresh token cookie exists', async () => {
      ...
      vi.mocked(authService.refreshTokenService).mockResolvedValue(
        serviceResult
      );
    ...
    it('should call next with error if refresh token service fails', async () => {
      ...
      const serviceError = new AuthError(
        'Invalid or expired refresh token',
        401
      );
      mockRequest.cookies = { refreshToken: oldRefreshToken };
      vi.mocked(authService.refreshTokenService).mockRejectedValue(
        serviceError
      );
  ...
  describe('requestPasswordReset', () => {
    it('should call requestPasswordResetService and return success message', async () => {
      ...
      vi.mocked(
        authService.requestPasswordResetService
      ...
      expect(
        authService.requestPasswordResetService
      ).toHaveBeenCalledWith(emailInput);
      expect(statusMock).toHaveBeenCalledWith(200);
  ...
  describe('resetPassword', () => {
    it('should call resetPasswordService and return success message', async () => {
      ...
      vi.mocked(authService.resetPasswordService).mockResolvedValue(
        serviceResult
      );
    ...
    it('should call next with error if service fails', async () => {
      ...
      mockRequest.body = resetInput;
      vi.mocked(authService.resetPasswordService).mockRejectedValue(
        serviceError
      );
