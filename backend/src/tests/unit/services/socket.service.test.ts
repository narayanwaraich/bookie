...
describe('Socket Service', () => {
  ...
  describe('emitToUser', () => {
    it('should emit event to the correct user room via io.to().emit()', () => {
      ...
      expect(mockedIo.to(userId).emit).toHaveBeenCalledWith(
        event,
        data
      );
      // Check logger call
      expect(mockLogger.info).toHaveBeenCalledWith(
        `[Socket Service]: Emitted event '${event}' to user room '${userId}'`
      );
    ...
    it('should log warning if userId is empty', () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        `[Socket Service]: Attempted to emit event '${event}' without a userId.`
      );
    ...
    it('should log warning if emit fails', () => {
      ...
      mockedIo.to.mockReturnValueOnce({
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        `[Socket Service]: Failed to emit event '${event}' to user room '${userId}' (adapter issue?).`
      );
      expect(mockLogger.info).not.toHaveBeenCalled();
      expect(mockLogger.error).not.toHaveBeenCalled();
    });
  ...
  describe('broadcastEvent', () => {
    it('should emit event directly via io.emit()', () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        `[Socket Service]: Broadcasted event '${event}' to all clients.`
      );
