...
vi.mock('../config/db', async () => ({
  default: {
    ...
    $transaction: vi
      .fn()
      .mockImplementation(async (callback) => callback(prisma)), // Mock transaction if used
  },
}));
...
describe('User Service', () => {
  ...
  describe('getUserProfile', () => {
    it('should return user profile if user is found', async () => {
      ...
      const expectedSelect = {
        ...
        lastLogin: true,
        isVerified: true,
        updatedAt: true, // Added updatedAt based on expectedProfile
        _count: {
          select: {
            bookmarks: { where: { isDeleted: false } },
            folders: true,
            tags: true,
            collections: true,
          },
      ...
      const mockUserWithCount = {
        ...mockUser,
        _count: { bookmarks: 0, folders: 0, tags: 0, collections: 0 },
      };
    ...
    it('should throw UserServiceError on database error', async () => {
      ...
      await expect(
        userService.getUserProfile(userId)
      ).rejects.toThrow('Failed to fetch user profile'); // Corrected message
  ...
  describe('updateUserProfile', () => {
    ...
    const expectedUpdatedProfile = {
      ...expectedProfile,
      ...updateData,
      updatedAt: updatedUserPartial.updatedAt,
      counts: { bookmarks: 0, folders: 0, tags: 0, collections: 0 },
    };
    ...
    it('should throw UserServiceError on database error during update', async () => {
      ...
      await expect(
        userService.updateUserProfile(userId, updateData)
      ).rejects.toThrow('Failed to update user profile'); // Corrected message
    });
  });
  ...
  describe('changeUserPassword', () => {
    ...
    it('should throw if current password does not match', async () => {
      ...
      ); // Keep mockImplementationOnce for override
      await expect(
        userService.changeUserPassword(userId, passwordData)
      ).rejects.toThrow('Current password is incorrect'); // Corrected message
    });
  ...
  describe('deleteUserAccount', () => {
    ...
    it('should throw if user not found', async () => {
      mockPrismaUser.findUnique.mockResolvedValue(null);
      await expect(
        userService.deleteUserAccount(userId, currentPassword)
      ).rejects.toThrow('User not found');
    });
    ...
    it('should throw if password does not match', async () => {
      ...
      await expect(
        userService.deleteUserAccount(userId, currentPassword)
      ).rejects.toThrow('Password is incorrect'); // Corrected message
    });
