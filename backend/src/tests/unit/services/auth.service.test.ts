...
import {
  ...
  beforeEach,
  afterEach,
} from 'vitest'; // Import vi explicitly
...
describe('Auth Service', () => {
  ...
  describe('getAuthenticatedUser', () => {
    ...
    it('should throw AuthError on database error', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        `Error fetching authenticated user ${userId}:`,
        dbError
      );
    });
  });
  ...
  describe('registerUser', () => {
    ...
    it('should register a new user successfully', async () => {
      ...
      expect(mockPrismaFolder.create).toHaveBeenCalledWith({
        data: { name: 'Favorites', userId: userId },
      });
  ...
  describe('loginUser', () => {
    ...
    const mockDbUser = createMockUser(userId, {
    ...
    it('should login user successfully', async () => {
      ...
      expect(mockPrismaUser.update).toHaveBeenCalledWith({
        where: { id: userId },
        data: { refreshToken, lastLogin: expect.any(Date) },
      }); // Use original token variable
      expect(result).toEqual({
    ...
    it('should throw AuthError if user not found', async () => {
      ...
      await expect(
        authService.loginUser(credentials)
      ).rejects.toThrow('Invalid credentials or user inactive');
    });
  ...
  describe('logoutUser', () => {
    ...
    it('should log error but not throw if DB update fails', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        `Error clearing refresh token for user ${userId} during logout:`,
        dbError
      );
    });
    ...
    it('should handle undefined userId gracefully', async () => {
      await expect(
        authService.logoutUser(undefined)
      ).resolves.toBeUndefined();
      expect(mockPrismaUser.update).not.toHaveBeenCalled();
      expect(mockLogger.warn).toHaveBeenCalledWith(
        'Logout called without user ID (likely already unauthenticated).'
      );
    });
  });
  ...
  describe('refreshTokenService', () => {
    ...
    it('should throw AuthError if token verification fails', async () => {
      ...
      await expect(
        authService.refreshTokenService(oldRefreshToken)
      ).rejects.toThrow(AuthError);
      await expect(
        authService.refreshTokenService(oldRefreshToken)
      ).rejects.toThrow('Invalid or expired refresh token');
      expect(mockLogger.error).toHaveBeenCalledWith(
        'Error verifying or refreshing token:',
        verifyError
      );
    });
    ...
    it('should throw AuthError if user not found or token mismatch', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        `Refresh token validation failed: User ${userId} not found or token mismatch.`
      );
    });
  });
  ...
  describe('requestPasswordResetService', () => {
    ...
    it('should return success message even if user does not exist', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        `Password reset requested for non-existent email: ${emailInput.email}`
      );
      expect(result).toEqual({
        message:
          'If an account with that email exists, a password reset link has been sent.',
    ...
    it('should return success message and log warning if email sending fails', async () => {
      ...
      expect(mockSendEmail).toHaveBeenCalled();
      expect(mockLogger.error).toHaveBeenCalledWith(
        `Error processing password reset request for ${emailInput.email}:`,
        emailError
      );
      expect(result).toEqual({
        message:
  ...
  describe('resetPasswordService', () => {
    ...
    const mockUser = createMockUser(userId, {
      passwordResetToken: resetInput.token,
    ...
    it('should throw AuthError if token is invalid or expired', async () => {
      mockPrismaUser.findFirst.mockResolvedValue(null);
      await expect(
        authService.resetPasswordService(resetInput)
      ).rejects.toThrow(AuthError);
      await expect(
        authService.resetPasswordService(resetInput)
