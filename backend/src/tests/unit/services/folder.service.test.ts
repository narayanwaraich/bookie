...
} from './folder.service'; // Import necessary constants/types
...
describe('Folder Service', () => {
  ...
  describe('createFolder', () => {
    ...
    it('should create a root folder successfully', async () => {
      ...
      const result = await folderService.createFolder(userId, {
        ...folderData,
        parentId: undefined,
      });
      ...
      expect(mockPrisma.folder.create).toHaveBeenCalledWith({
        data: { ...folderData, userId, parentId: null },
      });
    ...
    it('should throw FolderError if folder name already exists at the same level', async () => {
      ...
      await expect(
        folderService.createFolder(userId, {
          ...folderData,
          parentId: undefined,
        })
    ...
    it('should throw FolderError if parent folder does not exist', async () => {
      ...
      await expect(
        folderService.createFolder(userId, subfolderData)
      ).rejects.toThrow(FolderError);
      await expect(
        folderService.createFolder(userId, subfolderData)
  ...
  describe('getUserFolders', () => {
    ...
    const mockServiceResult = {
      data: [{ ...mockDbFolder, bookmarkCount: 5 }],
      totalCount: 1,
      page: 1,
      limit: 10,
      hasMore: false,
    ...
    it('should throw FolderError if cacheWrap returns null', async () => {
      ...
      await expect(
        folderService.getUserFolders(userId, queryParams)
      ).rejects.toThrow(FolderError);
      await expect(
        folderService.getUserFolders(userId, queryParams)
  ...
  describe('getFolderTree', () => {
    ...
    it('should throw FolderError if cacheWrap returns null', async () => {
      ...
      await expect(
        folderService.getFolderTree(userId)
      ).rejects.toThrow('Failed to fetch folder tree');
    });
  });
    ...
    it('should update folder successfully', async () => {
      ...
      expect(mockPrisma.folder.findFirst).toHaveBeenCalledWith({
        where: {
          ...
          userId,
          parentId: mockExistingFolder.parentId,
          id: { not: folderId },
          isDeleted: false,
        },
      });
      ...
      expect(mockInvalidateCache).toHaveBeenCalledWith(
        FOLDER_DETAIL_CACHE_KEY(folderId)
      );
      // Check socket emit
      expect(mockEmitToUser).toHaveBeenCalledWith(
        userId,
    ...
    it('should throw FolderError if name already exists at the same level', async () => {
      ...
      mockPrisma.folder.findFirst.mockResolvedValue(
        createMockFolder(
          'other-folder',
          userId,
          mockExistingFolder.parentId
        )
      );

      // Expect the service call to reject
      await expect(
        folderService.updateFolder(folderId, userId, updateData)
      ).rejects.toThrow(FolderError);
      await expect(
        folderService.updateFolder(folderId, userId, updateData)
      ).rejects.toThrow(
  ...
  describe('deleteFolder', () => {
    ...
    const mockDescendantFolder = createMockFolder(
      descendantId,
      userId,
      folderId
    );
    ...
    it('should soft delete folder and descendants successfully', async () => {
      ...
      expect(mockPrisma.folder.findMany).toHaveBeenCalledWith({
        where: { parentId: descendantId, userId },
        select: { id: true },
      });
      ...
      expect(mockInvalidateCache).toHaveBeenCalledWith(
        FOLDER_DETAIL_CACHE_KEY(descendantId)
      );
      // Check socket emit
      expect(mockEmitToUser).toHaveBeenCalledWith(
        userId,
        SOCKET_EVENTS.FOLDER_DELETED,
        { id: folderId, descendantIds: [descendantId] }
      ); // Reverted
    });
  });
  ...
  describe('Collaboration', () => {
    ...
    describe('addCollaborator', () => {
      beforeEach(() => {
        ...
        mockPrisma.folderCollaborator.create.mockResolvedValue(
          mockCollaboratorRecord as any
        );
      });

      it('should add a collaborator successfully', async () => {
        const result = await folderService.addCollaborator(
          folderId,
          userId,
          collaboratorUserId,
          Role.EDIT
        ...
        expect(
          mockPrisma.folderCollaborator.create
        ...
        expect(mockInvalidateCache).toHaveBeenCalledWith(
          FOLDER_TREE_CACHE_KEY(ownerId)
        );
        expect(mockInvalidateCachePattern).toHaveBeenCalledWith(
          `user:${ownerId}:folders:list:*`
        );
        expect(mockInvalidateCache).toHaveBeenCalledWith(
          FOLDER_TREE_CACHE_KEY(collaboratorUserId)
        );
        expect(mockInvalidateCachePattern).toHaveBeenCalledWith(
      ...
      it('should throw if user is not owner', async () => {
        ...
        ).rejects.toThrow('Only the owner can add collaborators');
      });
      ...
      it('should throw if collaborator user not found', async () => {
        ...
        ).rejects.toThrow('Collaborator user not found');
      });
      ...
      it('should throw if owner tries to add self', async () => {
        ...
        ).rejects.toThrow('Owner cannot be added as a collaborator');
      });
      ...
      it('should throw if user is already a collaborator (Prisma error)', async () => {
        ...
        mockPrisma.folderCollaborator.create.mockRejectedValue(
          prismaError
        );
        await expect(
          folderService.addCollaborator(
    ...
    describe('updateCollaboratorPermission', () => {
      beforeEach(() => {
        mockPrisma.folder.findUnique.mockResolvedValue(mockFolder);
        mockPrisma.folderCollaborator.update.mockResolvedValue({
          ...mockCollaboratorRecord,
      ...
      it('should update collaborator permission successfully', async () => {
        const result =
          await folderService.updateCollaboratorPermission(
            folderId,
            userId,
            collaboratorUserId,
            Role.VIEW
        ...
        expect(
          mockPrisma.folderCollaborator.update
        ).toHaveBeenCalledWith({
          where: {
      ...
      it('should throw if collaborator not found (Prisma error)', async () => {
        const prismaError = new Prisma.PrismaClientKnownRequestError(
          ...
          { code: 'P2025', clientVersion: 'x.y.z', meta: {} }
        ); // Added meta
        mockPrisma.folderCollaborator.update.mockRejectedValue(
          prismaError
        );
        await expect(
        ...
        ).rejects.toThrow('Collaborator not found on this folder');
      });
    ...
    describe('removeCollaborator', () => {
      beforeEach(() => {
        mockPrisma.folder.findUnique.mockResolvedValue(mockFolder);
        mockPrisma.folderCollaborator.delete.mockResolvedValue(
          {} as any
        );
      });

      it('should remove collaborator successfully', async () => {
        await folderService.removeCollaborator(
          folderId,
          userId,
          collaboratorUserId
        );
        expect(
          mockPrisma.folderCollaborator.delete
        ).toHaveBeenCalledWith({
          where: {
            folderId_userId: { folderId, userId: collaboratorUserId },
          },
      ...
      it('should throw if collaborator not found (Prisma error)', async () => {
        ...
        mockPrisma.folderCollaborator.delete.mockRejectedValue(
          prismaError
        );
        await expect(
          folderService.removeCollaborator(
            folderId,
            userId,
            collaboratorUserId
          )
