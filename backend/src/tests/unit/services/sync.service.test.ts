...
import {
  ...
  AnyClientChange,
  AnyServerRecord,
} from './sync.service'; // Import necessary types
...
describe('Sync Service', () => {
  ...
  const createMockBookmark = (
    ...
    favicon: null,
    previewImage: null,
    createdAt: new Date(updatedAt.getTime() - 5000),
    updatedAt,
    lastVisited: null,
    visitCount: 0,
    notes: null,
  ...
  it('should fetch server changes since last sync', async () => {
    const serverBookmark = createMockBookmark(
      'bm-server',
      userId,
      new Date(now.getTime() - 5000)
    );
    const serverFolder = createMockFolder(
      'f-server',
      userId,
      new Date(now.getTime() - 4000)
    );
    const serverTag = createMockTag(
      't-server',
      userId,
    ...
    mockPrisma.tag.findMany
      .mockResolvedValueOnce([serverTag])
      .mockResolvedValueOnce([]);
    mockPrisma.collection.findMany
      .mockResolvedValueOnce([serverCollection])
      .mockResolvedValueOnce([]);
    ...
    expect(result.deletedIds).toEqual({
      bookmarks: [deletedBookmarkId],
      folders: [deletedFolderId],
      tags: [],
      collections: [],
    });
  ...
  it('should process client creations', async () => {
    ...
    const createdBookmark = createMockBookmark(
      newBookmarkId,
      userId,
      now
    );
    ...
    expect(mockPrisma.bookmark.create).toHaveBeenCalledWith({
      data: expect.objectContaining({ id: newBookmarkId, userId }),
    });
    expect(mockPrisma.folder.create).toHaveBeenCalledWith({
      data: expect.objectContaining({ id: newFolderId, userId }),
    });
    expect(mockEmitToUser).toHaveBeenCalledWith(
      userId,
      SOCKET_EVENTS.BOOKMARK_CREATED,
      expect.objectContaining({ id: newBookmarkId })
  ...
  it('should process client updates', async () => {
    ...
    ); // Older than last sync
    const clientChange = {
      id: bookmarkId,
      title: 'Updated Client Title',
      updatedAt: now.toISOString(),
      lastServerUpdatedAt: existingBookmark.updatedAt.toISOString(),
    };
    const clientChanges: ClientChangesPayload = {
      bookmarks: [clientChange],
    };
    const updatedBookmark = {
      ...existingBookmark,
      title: clientChange.title,
      updatedAt: now,
    };
  ...
  it('should detect update conflict (server newer)', async () => {
    ...
    const clientChange = {
      id: bookmarkId,
      title: 'Client Conflict Title',
      updatedAt: now.toISOString(),
      lastServerUpdatedAt: new Date(
        now.getTime() - 5000
      ).toISOString(),
    }; // Client based on older version
    const clientChanges: ClientChangesPayload = {
      bookmarks: [clientChange],
    };
