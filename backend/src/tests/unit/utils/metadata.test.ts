...
describe('Metadata Utility', () => {
  ...
  it('should fetch and parse metadata successfully', async () => {
    // Arrange
    const mockHtml =
      '<html><head><title>Example Title</title><meta name="description" content="Example Desc"></head></html>';
    const mockResponse = {
      ok: true,
    ...
    expect(mockParse).toHaveBeenCalledWith(testUrl, {
      html: mockHtml,
    }); // Check parser called
  ...
  it('should return null if fetch response is not ok', async () => {
    ...
    expect(mockLoggerWarn).toHaveBeenCalledWith(
      `Failed to fetch metadata for ${testUrl}: Status ${404}`
    );
  });
  ...
  it('should return null if content-type is not HTML', async () => {
    ...
    expect(result).toBeNull();
    expect(mockLoggerWarn).toHaveBeenCalledWith(
      `Skipping metadata fetch for non-HTML content type: application/json for URL: ${testUrl}`
    );
  });
  ...
  it('should handle fetch error', async () => {
    ...
    expect(mockLoggerError).toHaveBeenCalledWith(
      `Error fetching URL ${testUrl} for metadata:`,
      fetchError
    );
  });
  ...
  it('should handle metadata parsing error', async () => {
    ...
    expect(result).toBeNull();
    expect(mockLoggerError).toHaveBeenCalledWith(
      `Error parsing metadata for ${testUrl}:`,
      parseError
    );
  });
  ...
  it('should extract favicon from parsed metadata if available', async () => {
    // Arrange
    const mockHtml =
      '<html><head><title>Title</title><link rel="icon" href="/favicon.ico"></head></html>';
    const mockResponse = {
      ok: true,
      text: vi.fn().mockResolvedValue(mockHtml),
