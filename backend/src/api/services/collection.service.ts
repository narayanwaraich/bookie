...
import {
  BOOKMARK_DETAIL_CACHE_KEY,
  USER_BOOKMARK_LIST_PATTERN,
...
// --- Cache Keys ---
export const COLLECTION_LIST_CACHE_KEY = (
  userId: string,
  queryParams: any
) => `user:${userId}:collections:list:${JSON.stringify(queryParams)}`;
export const COLLECTION_DETAIL_CACHE_KEY = (collectionId: string) =>
  `collection:${collectionId}:details`;
export const COLLECTION_PUBLIC_CACHE_KEY = (publicLink: string) =>
  `collection:public:${publicLink}`;
export const USER_COLLECTION_LIST_PATTERN = (userId: string) =>
  `user:${userId}:collections:list:*`;
 ...
 */
const checkCollectionPermission = async (
  collectionId: string,
  userId: string,
  ...
  logger.debug(
    `Checking access for user ${userId} on collection ${collectionId}, required: ${requiredPermission.join(
      '/'
    )}`
  );
  ...
  if (
    collection.isPublic &&
    requiredPermission.includes(Role.VIEW) &&
    requiredPermission.length === 1
  )
    return collection;
...
export const createCollection = async (
  data: CreateCollectionInput & { userId: string; ownerId: string }
): Promise<Collection> => {
  logger.info(
    `Creating collection "${data.name}" for user ${data.userId}`
  );
  const publicLink = data.isPublic ? uuidv4() : null;
  if (publicLink)
    logger.debug(
      `Generated public link for new collection: ${publicLink}`
    );

  try {
    const collection = await prisma.collection.create({
      data: { ...data, publicLink: publicLink },
    });
    logger.info(`Collection created successfully: ${collection.id}`);

    // --- Cache Invalidation ---
    await invalidateCachePattern(
      USER_COLLECTION_LIST_PATTERN(data.userId)
    );
...
export const getUserCollections = async (
  userId: string,
  queryParams: z.infer<typeof getCollectionsQuerySchema>
) => {
  ...
  const cachedResult = await cacheWrap(cacheKey, async () => {
    ...
    logger.debug(
      `Fetching collections (DB) for user ${userId} with limit: ${limit}, offset: ${offset}, sortBy: ${sortBy}, sortOrder: ${sortOrder}`
    );
    ...
    try {
      ...
      logger.info(
        `Found ${collections.length} collections (DB) (total: ${totalCount}) for user ${userId}`
      );
      return {
        data: collections,
        totalCount,
      ...
      logger.error(
        `Error fetching collections (DB) for user ${userId}:`,
        error
      );
      throw new CollectionError('Failed to fetch collections', 500);
    }
  });
...
export const getCollectionById = async (
  ...
  const cachedResult = await cacheWrap(cacheKey, async () => {
    logger.debug(
      `Fetching collection ${collectionId} (DB)` +
        (userId ? ` for user ${userId}` : ' (public access attempt)')
    );
    ...
    if (userId) {
      collectionMetadata = await checkCollectionPermission(
        collectionId,
        userId,
        [Role.VIEW, Role.EDIT, Role.ADMIN]
      );
    } else {
      collectionMetadata = await prisma.collection.findUnique({
        where: { id: collectionId, isPublic: true, isDeleted: false },
      });
    ...
    try {
      const whereBookmarks: Prisma.BookmarkWhereInput = {
        isDeleted: false,
        collections: { some: { collectionId } },
      };
      const [collectionDetails, bookmarks, totalBookmarkCount] =
      ...
      if (!collectionDetails)
        throw new CollectionError(
          'Collection not found during final fetch',
          404
        );
      return {
        ...collectionDetails,
        bookmarks: {
          data: bookmarks,
          totalCount: totalBookmarkCount,
          page: Math.floor(offset / limit) + 1,
          limit,
          hasMore: offset + bookmarks.length < totalBookmarkCount,
        },
      };
    } catch (error) {
      logger.error(
        `Error fetching full details (DB) for collection ${collectionId}:`,
        error
      );
      throw new CollectionError(
  ...
  if (cachedResult === null)
    throw new CollectionError('Failed to fetch collection', 500);
 ...
 */
export const getPublicCollectionByLink = async (
  publicLink: string
) => {
  ...
  const cachedResult = await cacheWrap(cacheKey, async () => {
    logger.debug(
      `Fetching public collection by link (DB): ${publicLink}`
    );
    const collection = await prisma.collection.findUnique({
    ...
    logger.info(
      `Public collection ${collection.id} fetched successfully (DB) via link.`
    );
    const {
      _count,
      bookmarks: bookmarkRelations,
      ...collectionData
    } = collection;
    return {
      ...collectionData,
      bookmarks: {
        data: bookmarkRelations.map((br) => br.bookmark),
        totalCount: _count?.bookmarks ?? 0,
        page: 1,
        limit: _count?.bookmarks ?? 0,
        hasMore: false,
      },
    };
  });
...
export const updateCollection = async (
  ...
  logger.debug(
    `User ${userId} attempting to update collection ${collectionId}`
  );
  await checkCollectionPermission(collectionId, userId, [
    Role.EDIT,
    Role.ADMIN,
  ]);
  logger.info(
    `Access granted for user ${userId} to update collection ${collectionId}.`
  );
  ...
  if (data.isPublic !== undefined) {
    const currentCollection = await prisma.collection.findUnique({
      where: { id: collectionId },
      select: { isPublic: true, publicLink: true },
    });
    if (!currentCollection)
      throw new CollectionError('Collection not found', 404);
    oldPublicLink = currentCollection.publicLink; // Store old link
    if (data.isPublic === true && !currentCollection.isPublic)
      publicLinkUpdate = currentCollection.publicLink || uuidv4();
    else if (data.isPublic === false && currentCollection.isPublic)
      publicLinkUpdate = null;
  }
  ...
  try {
    ...
    const updatedCollection = await prisma.collection.update({
      ...
      data: {
        ...
        ...(publicLinkUpdate !== undefined && {
          publicLink: publicLinkUpdate,
        }),
      },
    });
    ...
    if (oldPublicLink)
      await invalidateCache(
        COLLECTION_PUBLIC_CACHE_KEY(oldPublicLink)
      ); // Invalidate old public link cache
    if (updatedCollection.publicLink)
      await invalidateCache(
        COLLECTION_PUBLIC_CACHE_KEY(updatedCollection.publicLink)
      ); // Invalidate new public link cache
    await invalidateCachePattern(
    ...
    logger.error(
      `Error updating collection ${collectionId} in service:`,
      error
    );
    if (
      error instanceof Prisma.PrismaClientKnownRequestError &&
      ...
      throw new CollectionError(
        'Collection not found during update',
        404
      );
    else if (
      error instanceof Prisma.PrismaClientKnownRequestError &&
...
export const deleteCollection = async (
  ...
  logger.debug(
    `User ${userId} attempting to delete collection ${collectionId}`
  );
  const collection = await prisma.collection.findUnique({
    where: { id: collectionId },
  });
  ...
  if (collection.ownerId !== userId)
    throw new CollectionError(
      'Permission denied: Only the owner can delete a collection',
      403
    );
  logger.info(
    `Ownership verified for deletion of collection ${collectionId} by user ${userId}.`
  );
  ...
  try {
    ...
    logger.info(
      `Collection soft deleted successfully: ${collectionId}`
    );

    // --- Cache Invalidation ---
    await invalidateCache(COLLECTION_DETAIL_CACHE_KEY(collectionId));
    if (collection.publicLink)
      await invalidateCache(
        COLLECTION_PUBLIC_CACHE_KEY(collection.publicLink)
      );
    await invalidateCachePattern(
      USER_COLLECTION_LIST_PATTERN(userId)
    );
    ...
    emitToUser(getIO(), userId, SOCKET_EVENTS.COLLECTION_DELETED, {
      id: collectionId,
    }); // Use getIO()
  } catch (error: unknown) {
    logger.error(
      `Error deleting collection ${collectionId} in service:`,
      error
    );
    if (
      error instanceof Prisma.PrismaClientKnownRequestError &&
      ...
      throw new CollectionError(
        'Collection not found during delete',
        404
      );
    else if (error instanceof Error)
      throw new CollectionError(
...
export const addBookmarkToCollection = async (
  ...
  logger.debug(
    `User ${userId} attempting to add bookmark ${bookmarkId} to collection ${collectionId}`
  );
  const collection = await checkCollectionPermission(
    collectionId,
    userId,
    [Role.EDIT, Role.ADMIN]
  ); // Check edit access to collection
  logger.info(
    `Access granted for user ${userId} to modify collection ${collectionId}.`
  );
  ...
  try {
    const result = await prisma.bookmarkCollection.create({
      data: { collectionId, bookmarkId },
    });
    logger.info(
      `Bookmark ${bookmarkId} added successfully to collection ${collectionId}`
    );
    ...
    emitToUser(getIO(), userId, SOCKET_EVENTS.COLLECTION_UPDATED, {
      id: collectionId,
      addedBookmarkId: bookmarkId,
    }); // Use getIO()
    return result;
  } catch (error: unknown) {
    if (
      ...
      throw new CollectionError(
        'Bookmark already exists in this collection',
        409
      );
    }
    logger.error(
      `Error adding bookmark ${bookmarkId} to collection ${collectionId}:`,
      error
    );
    throw new CollectionError(
...
export const removeBookmarkFromCollection = async (
  ...
  logger.debug(
    `User ${userId} attempting to remove bookmark ${bookmarkId} from collection ${collectionId}`
  );
  const collection = await checkCollectionPermission(
    collectionId,
    userId,
  ...
  try {
    ...
    logger.info(
      `Bookmark ${bookmarkId} removed successfully from collection ${collectionId}`
    );
    ...
    if (
      ...
      throw new CollectionError(
        'Bookmark not found in this collection',
        404
      );
    }
    logger.error(
      `Error removing bookmark ${bookmarkId} from collection ${collectionId}:`,
      error
    );
    throw new CollectionError(
...
export const addCollaborator = async (
  ...
  logger.debug(
    `Owner ${ownerId} attempting to add collaborator ${collaboratorUserId} to collection ${collectionId} with permission ${permission}`
  );
  const collection = await prisma.collection.findUnique({
    where: { id: collectionId },
  });
  ...
  if (!collaboratorUser)
    throw new CollectionError('Collaborator user not found', 404);
  ...
  try {
    ...
    logger.info(
      `Collaborator ${collaboratorUserId} added successfully to collection ${collectionId} by owner ${ownerId}`
    );
    ...
    emitToUser(getIO(), ownerId, SOCKET_EVENTS.COLLECTION_UPDATED, {
      id: collectionId,
    ...
    emitToUser(
      ...
      collaboratorUserId,
      SOCKET_EVENTS.COLLECTION_UPDATED,
      { id: collectionId, collaborators: [collaborator] }
    ); // Use getIO()
    return collaborator;
  } catch (error: unknown) {
    if (
      ...
      throw new CollectionError(
        'User is already a collaborator on this collection',
        409
      );
    }
    logger.error(
      `Error adding collaborator ${collaboratorUserId} to collection ${collectionId}:`,
      error
    );
...
export const updateCollaboratorPermission = async (
  ...
  logger.debug(
    `Owner ${ownerId} attempting to update collaborator ${collaboratorUserId} permission to ${permission} on collection ${collectionId}`
  );
  const collection = await prisma.collection.findUnique({
    where: { id: collectionId },
  });
  ...
  if (ownerId === collaboratorUserId)
    throw new CollectionError('Cannot change owner permission', 400);

  try {
    const updatedCollaborator =
      await prisma.collectionCollaborator.update({
        where: {
          collectionId_userId: {
            collectionId,
    ...
    logger.info(
      `Collaborator ${collaboratorUserId} permission updated successfully to ${permission} in collection ${collectionId}`
    );
    ...
    emitToUser(
      ...
      collaboratorUserId,
      SOCKET_EVENTS.COLLECTION_UPDATED,
      { id: collectionId, collaborators: [updatedCollaborator] }
    ); // Use getIO()
    ...
    if (
      ...
      throw new CollectionError(
        'Collaborator not found on this collection',
        404
      );
    }
    logger.error(
      `Error updating permission for collaborator ${collaboratorUserId} on collection ${collectionId}:`,
      error
    );
    throw new CollectionError(
...
export const removeCollaborator = async (
  ...
  logger.debug(
    `Owner ${ownerId} attempting to remove collaborator ${collaboratorUserId} from collection ${collectionId}`
  );
  const collection = await prisma.collection.findUnique({
    where: { id: collectionId },
  });
  ...
  try {
    ...
    logger.info(
      `Collaborator ${collaboratorUserId} removed successfully from collection ${collectionId}`
    );
    ...
    emitToUser(
      ...
      { id: collectionId }
    ); // Notify collaborator they lost access // Use getIO()
  } catch (error: unknown) {
    if (
      error instanceof Prisma.PrismaClientKnownRequestError &&
      error.code === 'P2025'
    ...
    logger.error(
      `Error removing collaborator ${collaboratorUserId} from collection ${collectionId}:`,
      error
    );
