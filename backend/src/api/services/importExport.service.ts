  ...
  $(element)
    ...
    .each((_, dtElement) => {
      ...
      if (anchor.length > 0) {
        ...
        const addDate = addDateStr
          ? parseInt(addDateStr, 10)
          : undefined;
        const description =
          $(dtElement).next('dd').text().trim() || undefined;
 ...
 */
const parseHtmlBookmarks = (
  fileContent: string
): ParsedBookmark[] => {
  logger.info(
    '[Import Service]: Parsing HTML bookmarks hierarchically using cheerio.'
  );
  ...
  try {
    ...
    if (firstDl.length > 0 && firstDl.get(0)) {
      ...
      logger.warn(
        'Could not find standard starting <DL> after <H1> in HTML bookmark file.'
      );
      $('body > dl')
        .first()
        .each((_, dlElement) => {
    ...
    logger.info(
      `[Import Service]: Parsed ${bookmarks.length} bookmarks from HTML (hierarchical).`
    );
  } catch (error) {
    logger.error(
      '[Import Service]: Cheerio hierarchical HTML parsing error:',
      error
    );
  }
  return bookmarks;
...
const parseCsvBookmarks = (fileContent: string): ParsedBookmark[] => {
  logger.info(
    '[Import Service]: Parsing CSV bookmarks using papaparse.'
  );
  const bookmarks: ParsedBookmark[] = [];
  try {
    const result = papaparse.parse<Record<string, string>>(
      fileContent,
      {
        header: true,
        dynamicTyping: false,
    ...
    if (result.errors.length > 0) {
      logger.error(
        '[Import Service]: Papaparse encountered errors:',
        result.errors
      );
    }
    if (result.data && Array.isArray(result.data)) {
      result.data.forEach((row, index) => {
        ...
        if (url) {
          ...
          logger.warn(
            `[Import Service]: Skipping CSV row ${
              index + 2
            } due to missing URL.`
          );
        }
      ...
      logger.info(
        `[Import Service]: Parsed ${bookmarks.length} bookmarks from CSV.`
      );
    }
  } catch (error) {
    logger.error(
      '[Import Service]: Papaparse CSV parsing error:',
      error
    );
  }
  return bookmarks;
...
const parseJsonBookmarks = (jsonData: any): ParsedBookmark[] => {
  logger.info(
    '[Import Service]: Parsing JSON bookmarks (attempting hierarchical).'
  );
  const bookmarks: ParsedBookmark[] = [];
  const traverseNodes = (node: any, currentPath: string[]) => {
    ...
    if (type === 'folder' || (children && Array.isArray(children))) {
      ...
      if (url && typeof url === 'string' && url.startsWith('http')) {
        ...
        logger.warn(
          `[Import Service]: Skipping JSON item due to invalid/missing URL: ${
            url || 'N/A'
          }`
        );
      }
  ...
  logger.info(
    `[Import Service]: Parsed ${bookmarks.length} bookmarks from JSON (hierarchical).`
  );
  return bookmarks;
};
 ...
 */
const findOrCreateFolderByPath = async (
  userId: string,
  path: string[],
  ...
  logger.debug(
    `Finding/creating folder path: ${path.join(
      '/'
    )} for user ${userId} under parent ${baseParentId || 'root'}`
  );
  for (const folderName of path) {
    ...
    if (!folder) {
      logger.info(
        `Creating folder "${folderName}" under parent ${
      ...
      try {
        ...
        logger.error(
          `Failed to create folder "${folderName}" for user ${userId}:`,
          createError
        );
        if (
          ...
          if (!folder)
            throw new FolderError(
              `Failed to find or create folder "${folderName}" due to potential conflict.`,
              500
            );
        } else {
          throw new FolderError(
...
export const importBookmarks = async (
  ...
  logger.info(
    `Starting bookmark import for user ${userId}, format: ${format}` +
      (targetRootFolderId
        ? `, target root folder: ${targetRootFolderId}`
        : '')
  );
  ...
  try {
    switch (format) {
      case 'json':
        parsedBookmarks = parseJsonBookmarks(importData);
        break;
      case 'html':
        parsedBookmarks = parseHtmlBookmarks(importData as string);
        break;
      case 'csv':
        parsedBookmarks = parseCsvBookmarks(importData as string);
        break;
      default:
    ...
    logger.error(
      `[Import Service]: Error parsing ${format} data:`,
      error
    );
    if (error instanceof ImportExportError) throw error;
    throw new ImportExportError(
      `Failed to parse ${format} data. Please check the file format and content.`,
      400
    );
  }
  ...
  if (parsedBookmarks.length === 0) {
    logger.warn(
      `[Import Service]: No valid bookmarks found to import for user ${userId} from the provided ${format} file.`
    );
    return {
      success: true,
      message: 'No valid bookmarks found to import.',
      stats: { total: 0, imported: 0, failed: 0 },
    };
  }
  logger.info(
    `[Import Service]: Parsed ${parsedBookmarks.length} potential bookmarks. Starting database import for user ${userId}.`
  );

  // 2. Check target ROOT folder access if provided
  if (targetRootFolderId) {
    logger.debug(
      `Verifying target root folder ${targetRootFolderId} for user ${userId}`
    );
    ...
    if (!targetRootFolder) {
      logger.error(
        `Import failed: Target root folder ${targetRootFolderId} not found or permission denied for user ${userId}.`
      );
      throw new ImportExportError(
        'Target root folder not found or permission denied',
        404
      );
    }
    logger.debug(
      `Target root folder ${targetRootFolderId} verified.`
    );
  }
  ...
  for (const [index, bookmarkData] of parsedBookmarks.entries()) {
    try {
      if (
        !bookmarkData.url ||
        typeof bookmarkData.url !== 'string' ||
        !bookmarkData.url.startsWith('http')
      ) {
        logger.warn(
          `[Import Service]: Skipping bookmark #${
            index + 1
          } due to invalid or missing URL: ${bookmarkData.url}`
        );
        stats.failed++;
        continue;
      }
      ...
      const newBookmark = await prisma.$transaction(async (tx) => {
        const finalFolderId = await findOrCreateFolderByPath(
          userId,
          bookmarkData.folderPath || [],
          tx,
        ...
        const created = await tx.bookmark.create({
          data: {
            ...
            description,
            userId,
            createdAt: bookmarkData.addDate
              ? new Date(bookmarkData.addDate * 1000)
              : undefined,
          },
        });
        if (finalFolderId) {
          await tx.folderBookmark.create({
            data: { folderId: finalFolderId, bookmarkId: created.id },
          });
        }
        if (bookmarkData.tags && bookmarkData.tags.length > 0) {
          const tagOps = bookmarkData.tags.map(async (name) => {
            ...
            if (!tag) {
              try {
                tag = await tx.tag.create({ data: { name, userId } });
              } catch (e) {
                if (
            ...
            await tx.bookmarkTag.create({
              data: { tagId: tag.id, bookmarkId: created.id },
            });
          });
      ...
      if ((index + 1) % 100 === 0)
        logger.info(
          `[Import Service]: Imported ${stats.imported}/${stats.total} bookmarks for user ${userId}...`
        );
    } catch (error) {
      logger.error(
        `[Import Service]: Error importing individual bookmark #${
          index + 1
        } (URL: ${bookmarkData.url}):`,
        error
      );
  ...
  logger.info(
    `[Import Service]: Import completed for user ${userId}. Stats: Imported=${stats.imported}, Failed=${stats.failed}, Total=${stats.total}`
  );
  return {
    success: true,
    message: `Import completed. Imported ${stats.imported} bookmarks, failed ${stats.failed}.`,
    stats,
  };
};
 ...
 */
const buildHtmlFolderNode = (
  node: FolderHierarchyNode | null,
  indent: number = 0
): string => {
  if (!node) return '';
  let html = '';
  const indentStr = '  '.repeat(indent + 1);
  const escapedFolderName = (node.name || 'Unnamed Folder')
    .replace(/&/g, '&')
    .replace(/</g, '<')
    .replace(/>/g, '>')
    .replace(/"/g, '"');
  ...
  (node.bookmarks || []).forEach((b) => {
    ...
    const escapedTitle = (b.title || 'Untitled')
      .replace(/&/g, '&')
      .replace(/</g, '<')
      .replace(/>/g, '>')
      .replace(/"/g, '"');
    const tagsAttribute =
      b.tags.length > 0
        ...
        : '';
    const bookmarkIndent =
      node.id !== 'root' ? `${indentStr}  ` : indentStr;
    html += `${bookmarkIndent}<DT><A HREF="${b.url}" ADD_DATE="${addDate}"${tagsAttribute}>${escapedTitle}</A>\n`;
    if (b.description) {
      const escapedDescription = (b.description || '')
        .replace(/&/g, '&')
        .replace(/</g, '<')
        .replace(/>/g, '>')
        .replace(/"/g, '"');
  ...
  (node.children || []).forEach((childNode) => {
    html += buildHtmlFolderNode(
      childNode,
      node.id !== 'root' ? indent + 1 : indent
    );
  });
...
export const exportBookmarks = async (
  ...
  logger.info(
    `Starting bookmark export for user ${userId}, format: ${format}` +
  ...
  if (folderId) {
    const rootFolderAccess = await prisma.folder.findFirst({
      where: {
        id: folderId,
    ...
    if (includeSubfolders) {
      ...
      logger.info(
        `Including ${descendantIds.length} subfolders in export.`
      );
    }
    ...
    allRelevantFolders = await prisma.folder.findMany({
      where: { id: { in: targetFolderIds }, userId },
    });
  } else {
    // Exporting all bookmarks, fetch all user folders
    logger.debug(
      'Exporting all bookmarks, fetching all folders for hierarchy.'
    );
    allRelevantFolders = await prisma.folder.findMany({
      where: { userId },
    });
  ...
  if (targetFolderIds) {
    whereClause.folders = {
      some: { folderId: { in: targetFolderIds } },
    };
  }

  // 3. Fetch bookmarks with relations
  logger.debug(
    `Fetching bookmarks for export for user ${userId} with where clause: ${JSON.stringify(
      whereClause
    )}`
  );
  ...
  logger.info(
    `Found ${bookmarks.length} bookmarks to export for user ${userId}.`
  );
  ...
  bookmarks.forEach((b) => {
    ...
    if (b.folders.length > 0) {
      b.folders.forEach((fb) => {
        ...
        if (folderMFE) {
          ...
          folderMFE.bookmarks.push(
            b as unknown as Bookmark & {
    ...
    if (
      !placedInFolder &&
      !rootFolderIdForHierarchy &&
      folderMapForExport[VIRTUAL_ROOT_ID]
    ) {
      // Use type assertion carefully
      folderMapForExport[VIRTUAL_ROOT_ID].bookmarks.push(
        b as unknown as Bookmark & {
          tags: { tag: { name: string } }[];
        }
      );
  ...
  try {
    ...
    const rootNode =
      folderMapForExport[rootFolderIdForHierarchy || VIRTUAL_ROOT_ID];
    // Add a check for rootNode existence before proceeding with hierarchical formats
    if (!rootNode && (format === 'html' || format === 'json')) {
      logger.error(
        'Root node for hierarchy not found during export formatting.'
      );
      throw new ImportExportError(
        'Failed to build export hierarchy.',
        500
      );
    }
      ...
      switch (format) {
        case 'json':
          ...
          const jsonOutput = {
            ...
            roots: { bookmarks_bar: jsonRootNode },
            version: 1,
          };
          return {
          ...
          const rows = bookmarks.map((b) =>
            [
              ...
              escapeCsv(
                b.folders
                  .map(
                    (f) => folderMapForExport[f.folder.id]?.name || ''
                  )
                  .filter((name) => name)
                  .join(';')
              ),
    ...
    logger.error(
      `[Export Service]: Error formatting data for ${format}:`,
      error
    );
    throw new ImportExportError(
      `Failed to format data for ${format}`,
      500
    );
  }
};
