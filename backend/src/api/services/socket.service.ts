...
export const emitToUser = (
  ...
  try {
    ...
    logger.error(
      '[Socket Service]: Failed to get IO instance in emitToUser:',
      error
    );
    return;
  }
  if (!userId) {
    logger.warn(
      `[Socket Service]: Attempted to emit event '${eventName}' without a userId.`
    );
    return;
  }
  ...
  if (result) {
    // Check if emit was potentially successful (at least to the adapter)
    logger.info(
      `[Socket Service]: Emitted event '${eventName}' to user room '${roomName}'`
    );
  } else {
    logger.warn(
      `[Socket Service]: Failed to emit event '${eventName}' to user room '${roomName}' (adapter issue?).`
    );
  }
...
export const broadcastEvent = (
  ...
  try {
    ...
    logger.error(
      '[Socket Service]: Failed to get IO instance in broadcastEvent:',
      error
    );
    return;
  }
  ...
  logger.info(
    `[Socket Service]: Broadcasted event '${eventName}' to all clients.`
  );
};
