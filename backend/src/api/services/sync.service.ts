...
export type ClientBookmarkChange = {
  // Exported
  id: string;
  url?: string;
  title?: string;
...
export type ClientFolderChange = {
  // Exported
  id: string;
  name?: string;
...
export type ClientCollectionChange = {
  // Exported
  id: string;
  name?: string;
  description?: string | null;
...
export type ClientChangesPayload = {
  // Exported
...
export type AnyClientChange =
  | ClientBookmarkChange
  | ClientFolderChange
  | ClientTagChange
  | ClientCollectionChange; // Exported
...
const syncBookmarkRelations = async (
  ...
  if (clientFolderIds !== undefined) {
    const currentFolders = await tx.folderBookmark.findMany({
      where: { bookmarkId },
      select: { folderId: true },
    });
    const currentFolderIdSet = new Set(
      currentFolders.map((f) => f.folderId)
    );
    const clientFolderIdSet = new Set(clientFolderIds);
    const foldersToAdd = clientFolderIds.filter(
      (id) => !currentFolderIdSet.has(id)
    );
    const foldersToRemove = currentFolders
      .filter((f) => !clientFolderIdSet.has(f.folderId))
      .map((f) => f.folderId);
    if (foldersToAdd.length > 0) {
      const accessibleFolders = await tx.folder.count({
        where: { id: { in: foldersToAdd }, userId },
      });
      if (accessibleFolders !== foldersToAdd.length)
        throw new Error(
          `Permission denied for adding bookmark ${bookmarkId} to one or more folders.`
        );
      await tx.folderBookmark.createMany({
        data: foldersToAdd.map((folderId) => ({
          bookmarkId,
          folderId,
        })),
        skipDuplicates: true,
    ...
    if (foldersToRemove.length > 0)
      await tx.folderBookmark.deleteMany({
        where: { bookmarkId, folderId: { in: foldersToRemove } },
      });
  }
  // Tag Sync Logic
  if (clientTagIds !== undefined) {
    const currentTags = await tx.bookmarkTag.findMany({
      where: { bookmarkId },
      select: { tagId: true },
    });
    const currentTagIdSet = new Set(currentTags.map((t) => t.tagId));
    const clientTagIdSet = new Set(clientTagIds);
    const tagsToAdd = clientTagIds.filter(
      (id) => !currentTagIdSet.has(id)
    );
    const tagsToRemove = currentTags
      .filter((t) => !clientTagIdSet.has(t.tagId))
      .map((t) => t.tagId);
    if (tagsToAdd.length > 0) {
      const accessibleTags = await tx.tag.count({
        where: { id: { in: tagsToAdd }, userId },
      });
      if (accessibleTags !== tagsToAdd.length)
        throw new Error(
          `One or more tags not found or not owned by user for bookmark ${bookmarkId}.`
        );
      await tx.bookmarkTag.createMany({
        data: tagsToAdd.map((tagId) => ({ bookmarkId, tagId })),
        skipDuplicates: true,
      });
    }
    if (tagsToRemove.length > 0)
      await tx.bookmarkTag.deleteMany({
        where: { bookmarkId, tagId: { in: tagsToRemove } },
      });
  }
};
...
const getAllDescendantFolderIds = async (
  ...
  for (const childId of childIds) {
    const descendants = await getAllDescendantFolderIds(
      childId,
      userId,
      txOrPrisma
    );
...
export const syncData = async (
  ...
  clientChangesPayload: ClientChangesPayload
  ...
  const lastSyncDate = lastSyncTimestamp
    ...
    : new Date(0);
  logger.info(
    `[Sync Service] Starting sync for user ${userId}. Last sync: ${
  ...
  try {
    ...
    logger.debug(
      `[Sync Service] Fetching server changes since ${lastSyncDate.toISOString()}`
    );
    ...
    const updatedBookmarks = await prisma.bookmark.findMany({
      ...
      include: {
        ...
        folders: { select: { folder: { select: { id: true } } } },
      },
    });
    if (updatedBookmarks.length > 0)
      serverChanges.push(
        ...updatedBookmarks.map((b) => ({ type: 'bookmark', ...b }))
      );
    const deletedBookmarks = await prisma.bookmark.findMany({
      where: {
        userId,
        isDeleted: true,
    ...
    if (deletedBookmarks.length > 0)
      deletedIds.bookmarks.push(...deletedBookmarks.map((b) => b.id));
    ...
    if (updatedFolders.length > 0)
      serverChanges.push(
        ...updatedFolders.map((f) => ({ type: 'folder', ...f }))
      );
    ...
    if (deletedFolders.length > 0)
      deletedIds.folders.push(...deletedFolders.map((f) => f.id));
    ...
    if (deletedTags.length > 0)
      deletedIds.tags.push(...deletedTags.map((t) => t.id));
    ...
    if (deletedCollections.length > 0)
      deletedIds.collections.push(
        ...deletedCollections.map((c) => c.id)
      );

    logger.info(
      `[Sync Service] Found server changes - Bookmarks: ${updatedBookmarks.length}, Folders: ${updatedFolders.length}, Tags: ${updatedTags.length}, Collections: ${updatedCollections.length}.`
    );
    logger.info(
      `[Sync Service] Found server deletions - Bookmarks: ${deletedIds.bookmarks.length}, Folders: ${deletedIds.folders.length}, Tags: ${deletedIds.tags.length}, Collections: ${deletedIds.collections.length}.`
    );

    // --- 2. Process Client Changes ---

    // Process Bookmarks
    const clientBookmarkChanges =
      clientChangesPayload.bookmarks || [];
    if (clientBookmarkChanges.length > 0) {
      logger.info(
        `[Sync Service] Processing ${clientBookmarkChanges.length} client bookmark changes.`
      );
      for (const change of clientBookmarkChanges) {
        ...
        try {
            ...
            if (change.isDeleted) {
              if (
                ...
                await tx.bookmark.update({
                  where: { id: change.id },
                  data: { isDeleted: true, deletedAt: syncStartTime },
                });
              ...
              const clientLastKnown = change.lastServerUpdatedAt
                ? new Date(change.lastServerUpdatedAt)
                : new Date(0);
              if (existing.updatedAt > clientLastKnown) {
                conflicts.push({
                  type: 'bookmark',
                  clientChange: change,
                  serverRecord: existing,
              ...
              const updated = await tx.bookmark.findUnique({
                where: { id: change.id },
                include: {
                  tags: { select: { tag: { select: { id: true } } } },
                  folders: {
                    select: { folder: { select: { id: true } } },
                  },
                },
              });
              if (updated)
          ...
          logger.error(
            `[Sync Service] Failed processing bookmark change ${change.id}:`,
            error
          );
          if (error instanceof Error)
            conflicts.push({
              type: 'bookmark',
              clientChange: change,
              serverRecord: { error: error.message },
            });
        }
      }
    }
    ...
    if (clientFolderChanges.length > 0) {
      logger.info(
        `[Sync Service] Processing ${clientFolderChanges.length} client folder changes.`
      );
      for (const change of clientFolderChanges) {
        ...
        try {
          await prisma.$transaction(async (tx) => {
            ...
            if (change.isDeleted) {
              if (
                ...
                logger.warn(
                  `[Sync Tx] Soft deleting folder ${change.id}. Bookmarks inside might become orphaned if not moved separately.`
                );
              ...
              if (
                ...
                change.parentId !== existing.parentId
              ) {
              ...
              if (
                (change.name && change.name !== existing.name) ||
                (change.parentId !== undefined &&
                  change.parentId !== existing.parentId)
              ) {
                ...
                if (duplicate)
                  throw new FolderError(
                    `Folder name "${nameToCheck}" already exists at this level`,
                    409
                  );
              }
              ...
              }); // Ensure delete flags are reset on update
              emitToUser(
                userId,
                SOCKET_EVENTS.FOLDER_UPDATED,
                updated
              );
            } else {
              // Create
              ...
              if (change.parentId) {
                const parentExists = await tx.folder.count({
                  where: {
                    id: change.parentId,
                    userId,
                    isDeleted: false,
                  },
                }); // Check parent exists and isn't deleted
                if (parentExists === 0)
                  throw new FolderError(
                    'Parent folder not found or is deleted',
                    404
                  );
              }
          ...
          logger.error(
            `[Sync Service] Failed processing folder change ${change.id}:`,
            error
          );
          if (error instanceof Error)
            conflicts.push({
    ...
    if (clientTagChanges.length > 0) {
      logger.info(
        `[Sync Service] Processing ${clientTagChanges.length} client tag changes.`
      );
      for (const change of clientTagChanges) {
        ...
        try {
          await prisma.$transaction(async (tx) => {
            ...
            if (change.isDeleted) {
              if (
                ...
                emitToUser(userId, SOCKET_EVENTS.TAG_DELETED, {
                  id: change.id,
                });
              }
            } else if (existing) {
              ...
              if (change.name && change.name !== existing.name) {
                ...
                if (duplicate)
                  throw new TagError('Tag name already exists', 409);
              }
              ...
              // Create
              const duplicate = await tx.tag.findFirst({
                where: { name: change.name || 'Untitled', userId },
              });
              if (duplicate)
                throw new TagError('Tag name already exists', 409);
          ...
          logger.error(
            `[Sync Service] Failed processing tag change ${change.id}:`,
            error
          );
          if (error instanceof Error)
            conflicts.push({
    ...
    // Process Collections
    const clientCollectionChanges =
      clientChangesPayload.collections || [];
    if (clientCollectionChanges.length > 0) {
      logger.info(
        `[Sync Service] Processing ${clientCollectionChanges.length} client collection changes.`
      );
      for (const change of clientCollectionChanges) {
        ...
        try {
          await prisma.$transaction(async (tx) => {
            ...
            if (change.isDeleted) {
              if (
                ...
                // Assuming owner deletes
                await tx.collection.update({
                  where: { id: change.id },
                  data: { isDeleted: true, deletedAt: syncStartTime },
                });
                emitToUser(userId, SOCKET_EVENTS.COLLECTION_DELETED, {
                  id: change.id,
                });
              ...
              let publicLinkUpdate: string | null | undefined =
                undefined;
              if (
                change.isPublic !== undefined &&
                change.isPublic !== existing.isPublic
              ) {
                publicLinkUpdate = change.isPublic
                  ? existing.publicLink || uuidv4()
                  : null;
              }
              ...
              const created = await tx.collection.create({
                data: {
                  ...
                  name: change.name || 'Untitled',
                  description: change.description,
                  isPublic: change.isPublic ?? false,
                  thumbnail: change.thumbnail,
                  publicLink,
                  userId,
                  ownerId: userId,
          ...
          logger.error(
            `[Sync Service] Failed processing collection change ${change.id}:`,
            error
          );
          if (error instanceof Error)
            conflicts.push({
    ...
    const finalServerChanges = serverChanges.filter((sc) => {
      switch (sc.type) {
        case 'bookmark':
          return !processedClientIds.bookmarks.has(sc.id);
        case 'folder':
          return !processedClientIds.folders.has(sc.id);
        case 'tag':
          return !processedClientIds.tags.has(sc.id);
        case 'collection':
          return !processedClientIds.collections.has(sc.id);
        default:
          logger.warn(
            `[Sync Service] Unknown server change type found during filtering: ${sc.type}`
          );
          return true;
      }
    });
    logger.info(
      `[Sync Service] Sync finished for user ${userId}. Sending ${
        finalServerChanges.length
      } server changes, deletions: ${JSON.stringify(deletedIds)}, ${
        conflicts.length
      } conflicts.`
    );
    ...
    logger.error(
      `[Sync Service] Sync failed catastrophically for user ${userId}:`,
      error
    );
    return {
      success: false,
