  ...
  try {
    const user = await prisma.user.findUnique({
      ...
      select: {
        ...
        _count: {
          select: {
            // Select counts of related items
...
export const updateUserProfile = async (
  ...
  try {
    ...
    if (currentPassword && newPassword) {
      logger.info(
        `User ${userId} requested password change during profile update.`
      );
      ...
      logger.warn(
        `Update profile failed for user ${userId}: Incomplete password change data provided.`
      );
    ...
    if (username && username !== user.username) {
      logger.debug(
        `User ${userId} attempting to change username to ${username}. Checking availability.`
      );
      ...
      if (existingUser) {
        logger.warn(
          `Update profile failed for user ${userId}: Username ${username} already taken.`
        );
        throw new UserServiceError('Username already taken', 409); // Conflict
      }
      updateData.username = username;
      logger.info(
        `Username for user ${userId} will be updated to ${username}.`
      );
    }
    ...
    if (Object.keys(updateData).length > 0) {
      logger.debug(
        `Updating profile fields for user ${userId}: ${Object.keys(
          updateData
        ).join(', ')}`
      );
    ...
    if (
      ...
      logger.warn(
        `Update profile failed for user ${userId} due to unique constraint.`
      );
...
export const changeUserPassword = async (
  ...
  try {
    ...
    if (!user) {
      logger.warn(
        `Change password failed: User ${userId} not found.`
      );
      throw new UserServiceError('User not found', 404);
    }
    ...
    if (!isMatch) {
      logger.warn(
        `Change password failed for user ${userId}: Current password incorrect.`
      );
      throw new UserServiceError(
    ...
    logger.info(
      `Password changed successfully for user ${userId}. Refresh token invalidated.`
    );
    return {
      success: true,
...
export const deleteUserAccount = async (
  ...
  try {
    ...
    if (!currentPassword) {
      logger.warn(
        `Delete account failed for user ${userId}: Password verification missing.`
      );
      throw new UserServiceError(
    ...
    if (!isMatch) {
      logger.warn(
        `Delete account failed for user ${userId}: Password incorrect.`
      );
      throw new UserServiceError('Password is incorrect', 400);
    }

    logger.info(
      `Password verified for account deletion of user ${userId}. Proceeding with deletion.`
    );
