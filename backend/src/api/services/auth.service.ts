...
export const registerUser = async (userData: RegisterUserInput) => {
  ...
  const user = await prisma.$transaction(async (tx) => {
    ...
    logger.info(
      `Default 'Favorites' folder created for user ID: ${newUser.id}`
    );
...
export const loginUser = async (credentials: LoginUserInput) => {
  ...
  if (!user || !user.isActive) {
    logger.warn(
      `Login failed for ${email}: User not found or inactive.`
    );
  ...
  logger.info(
    `User ${user.username} logged in successfully. Updated refresh token and last login.`
  );
...
export const requestPasswordResetService = async (
  emailInput: ResetPasswordRequestInput
) => {
  ...
  if (user) {
    try {
      ...
      logger.debug(
        `Generated password reset token for user ${user.id}`
      );
...
export const resetPasswordService = async (
  ...
  logger.info(
    `Password reset successful for user ${user.id}. Refresh tokens invalidated.`
  );
...
export const refreshTokenService = async (
  ...
  try {
    ...
    if (!user) {
      logger.warn(
        `Refresh token validation failed: User ${decoded.id} not found or token mismatch.`
      );
    ...
    logger.info(
      `Generated new tokens for user ${user.id} during refresh.`
    );
...
export const verifyUserEmail = async (token: string) => {
  ...
  if (user.isVerified) {
    logger.info(
      `Email verification attempt for already verified user: ${user.id}`
    );
...
export const getAuthenticatedUser = async (userId: string) => {
  logger.debug(
    `Fetching authenticated user profile for ID: ${userId}`
  );
  try {
    const user = await prisma.user.findUnique({
      where: { id: userId },
    ...
    if (!user) {
      ...
      logger.error(
        `Authenticated user check failed: User not found for ID ${userId}`
      );
