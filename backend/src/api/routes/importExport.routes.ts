...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    const allowedTypes = [
      ...
      'application/json',
      'text/csv',
      'application/x-chrome-bookmarks',
    ];
    ...
    if (allowedTypes.includes(file.mimetype)) {
      ...
      cb(
        new Error(
          'Unsupported file type. Please upload HTML, JSON, or CSV.'
        )
      );
    }
  },
...
const importExportLimiter = rateLimit({
  ...
  legacyHeaders: false,
  message:
    'Too many import/export requests from this IP, please try again after 15 minutes',
});
