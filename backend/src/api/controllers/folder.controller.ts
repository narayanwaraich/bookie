...
export const getFolders = async (
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      logger.error(
        'Get folders query validation error:',
        error.errors
      );
    } else {
      logger.error(
        `Error fetching folders for user ${userId}:`,
        error
      );
    }
    next(error);
...
export const getFolderTree = async (
  ...
  try {
    ...
    logger.error(
      `Error fetching folder tree for user ${userId}:`,
      error
    );
    next(error);
  }
...
export const getFolder = async (
  ...
  try {
    const userId = req.user!.id;
    if (!folderId)
      throw new FolderError('Folder ID is required.', 400);
...
export const deleteFolder = async (
  ...
  try {
    ...
    const { moveBookmarksTo } = deleteFolderQuerySchema.parse(
      req.query
    );
    if (!folderId)
      throw new FolderError('Folder ID is required.', 400);
    ...
    if (error instanceof z.ZodError) {
      logger.error(
        'Delete folder query validation error:',
        error.errors
      );
    } else {
...
export const getBookmarksByFolder = async (
  ...
  try {
    ...
    const queryParams = getBookmarksInContainerQuerySchema.parse(
      req.query
    );

    // Ownership/access checked by middleware before calling service
    const result = await folderService.getBookmarksByFolder(
      folderId,
      userId,
      queryParams
    );
    ...
    if (error instanceof z.ZodError) {
      logger.error(
        `Get bookmarks by folder query validation error for folder ${folderId}:`,
        error.errors
      );
    } else {
      logger.error(
        `Error fetching bookmarks for folder ${folderId}:`,
        error
      );
    }
    next(error);
...
export const addBookmarkToFolder = async (
  ...
  try {
    const userId = req.user!.id;
    // Validate body
    const { bookmarkId } = addBookmarkToFolderBodySchema.parse(
      req.body
    );
    ...
    logger.error(
      `Error adding bookmark to folder ${folderId}:`,
      error
    );
    next(error);
  }
  ...
  try {
    ...
    if (!folderId || !bookmarkId) {
      throw new FolderError(
        'Folder ID and Bookmark ID are required in route parameters',
        400
      );
    }
    // Ownership/access checked by middleware/service
 ...
 *               $ref: '#/components/schemas/ErrorResponse'
 */
export const addCollaboratorToFolder = async (
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
) => {
  ...
  try {
    ...
    const {
      ...
      permission,
    }: AddFolderCollaboratorInput = req.body;

    if (!folderId)
      throw new FolderError('Folder ID is required.', 400);
    // Ownership checked by middleware/service
    const collaborator = await folderService.addCollaborator(
      folderId,
      ownerId,
      collaboratorUserId,
      permission
    ...
    logger.info(
      `Collaborator ${collaboratorUserId} added to folder ${folderId} by owner ${ownerId}`
    );
    res.status(201).json({ success: true, data: collaborator });
  } catch (error) {
    logger.error(
      `Error adding collaborator to folder ${folderId}:`,
      error
    );
    next(error);
  }
 ...
 *               $ref: '#/components/schemas/ErrorResponse'
 */
export const updateFolderCollaborator = async (
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
) => {
  ...
  try {
    ...
    if (!folderId || !collaboratorId)
      throw new FolderError(
        'Folder ID and Collaborator ID are required in route parameters',
        400
      );
    // Ownership checked by middleware/service
    ...
    logger.info(
      `Collaborator ${collaboratorId} permission updated in folder ${folderId} by owner ${ownerId}`
    );
    res
      .status(200)
      .json({ success: true, data: updatedCollaborator });
  } catch (error) {
    logger.error(
      `Error updating collaborator ${collaboratorId} in folder ${folderId}:`,
      error
    );
    next(error);
  }
 ...
 *               $ref: '#/components/schemas/ErrorResponse'
 */
export const removeFolderCollaborator = async (
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
) => {
  ...
  try {
    ...
    logger.info(
      `Collaborator ${collaboratorId} removed from folder ${folderId} by owner ${ownerId}`
    );
    res.status(204).send(); // No Content
  } catch (error) {
    logger.error(
      `Error removing collaborator ${collaboratorId} from folder ${folderId}:`,
      error
    );
    next(error);
  }
