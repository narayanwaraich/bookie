...
export const createCollection = async (
  ...
  try {
    ...
    logger.info(
      `Collection created: ${newCollection.id} by user ${userId}`
    );
...
export const getUserCollections = async (
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      logger.error(
        'Get collections query validation error:',
        error.errors
      );
    } else {
      logger.error(
        `Error fetching collections for user ${userId}:`,
        error
      );
    }
    next(error);
...
export const getCollectionById = async (
  ...
  try {
    ...
    if (!collectionId)
      throw new CollectionError('Collection ID is required.', 400);
    ...
    if (error instanceof z.ZodError) {
      logger.error(
        `Get collection by ID query validation error for collection ${collectionId}:`,
        error.errors
      );
    } else {
      logger.error(
...
export const getPublicCollectionByLink = async (
  ...
  try {
    if (!publicLink)
      throw new CollectionError('Public link is required.', 400);
    ...
    logger.error(
      `Error fetching public collection by link ${publicLink}:`,
      error
    );
    next(error);
  }
...
export const updateCollection = async (
  ...
  try {
    ...
    logger.info(
      `Collection updated: ${collectionId} by user ${userId}`
    );
...
export const deleteCollection = async (
  ...
  try {
    ...
    logger.info(
      `Collection deleted: ${collectionId} by user ${userId}`
    );
...
export const addBookmarkToCollection = async (
  ...
  try {
    ...
    const result = await collectionService.addBookmarkToCollection(
      collectionId,
      bookmarkId,
      userId
    );
    ...
    logger.error(
      `Error adding bookmark to collection ${collectionId}:`,
      error
    );
    next(error);
  }
  ...
  try {
    ...
    if (!collectionId || !bookmarkId)
      throw new CollectionError(
        'Collection ID and Bookmark ID are required',
        400
      );
    // Ownership/edit permission checked by middleware/service
 ...
 *               $ref: '#/components/schemas/ErrorResponse'
 */
export const addCollaboratorToCollection = async (
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
) => {
  ...
  try {
    ...
    const {
      ...
      permission,
    }: AddCollectionCollaboratorInput = req.body;
    ...
    logger.info(
      `Collaborator ${collaboratorUserId} added to collection ${collectionId} by owner ${ownerId}`
    );
    res.status(201).json({ success: true, data: collaborator });
  } catch (error) {
    logger.error(
      `Error adding collaborator to collection ${collectionId}:`,
      error
    );
    next(error);
  }
...
export const updateCollaboratorPermission = async (
  ...
  try {
    const ownerId = req.user!.id;
    // Validate body
    const { permission }: UpdateCollectionCollaboratorInput =
      req.body;

    if (!collectionId || !collaboratorId)
      throw new CollectionError(
        'Collection ID and Collaborator ID are required',
        400
      );
    // Ownership checked by middleware/service
    ...
    logger.info(
      `Collaborator ${collaboratorId} permission updated in collection ${collectionId} by owner ${ownerId}`
    );
    res
      .status(200)
    ...
    logger.error(
      `Error updating collaborator ${collaboratorId} in collection ${collectionId}:`,
      error
    );
    next(error);
  }
 ...
 *               $ref: '#/components/schemas/ErrorResponse'
 */
export const removeCollaboratorFromCollection = async (
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
) => {
  ...
  try {
    ...
    logger.info(
      `Collaborator ${collaboratorId} removed from collection ${collectionId} by owner ${ownerId}`
    );
    res.status(204).send(); // No Content
  } catch (error) {
    logger.error(
      `Error removing collaborator ${collaboratorId} from collection ${collectionId}:`,
      error
    );
    next(error);
  }
