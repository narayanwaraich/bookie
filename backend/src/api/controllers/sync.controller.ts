...
import {
  SyncRequestBodyInput,
  syncRequestBodySchema,
} from '../models/schemas'; // Import schema and type
...
export const handleSync = async (
  ...
  try {
    ...
    const { lastSyncTimestamp, clientChanges }: SyncRequestBodyInput =
      req.body;
    ...
    logger.info(
      `[Sync Controller]: Received sync request from user ${userId}, last sync: ${
        lastSyncTimestamp || 'Never'
      }. Client changes - Bookmarks: ${bookmarkCount}, Folders: ${folderCount}, Tags: ${tagCount}, Collections: ${collectionCount}`
    );
    ...
    if (!syncResult.success) {
      ...
      logger.warn(
        `[Sync Controller]: Sync service reported failure for user ${userId}. Message: ${syncResult.message}`
      );
    ...
    logger.info(
      `[Sync Controller]: Sync completed successfully for user ${userId}. Sending response.`
    );
    ...
    if (error instanceof z.ZodError) {
      logger.error(
        `[Sync Controller] Sync request body validation error for user ${userId}:`,
        error.errors
      );
    } else {
      logger.error(
        `[Sync Controller]: Unhandled error during sync for user ${userId}:`,
        error
      );
    }
