...
export const getBookmark = async (
  ...
  try {
    ...
    if (!bookmarkId)
      throw new BookmarkError('Bookmark ID is required.', 400);
    ...
    logger.error(
      `Error in getBookmark controller for ID ${bookmarkId}:`,
      error
    );
    next(error);
  }
...
export const updateBookmark = async (
  ...
  try {
    ...
    logger.error(
      `Error in updateBookmark controller for ID ${bookmarkId}:`,
      error
    );
    next(error);
  }
...
export const deleteBookmark = async (
  ...
  try {
    ...
    logger.error(
      `Error in deleteBookmark controller for ID ${bookmarkId}:`,
      error
    );
    next(error);
  }
...
export const searchBookmarks = async (
  ...
  try {
    ...
    const validatedQueryParams = bookmarkSearchSchema.parse(
      req.query
    );
...
export const checkBookmarkUrlExists = async (
  ...
  try {
    ...
    logger.debug(
      `Checking existence for URL: ${url} for user ${userId}`
    );
    ...
    if (error instanceof z.ZodError) {
      ...
      logger.error(
        'Error in checkBookmarkUrlExists controller:',
        error
      );
    }
    next(error);
  ...
  try {
    ...
    if (!bookmarkId)
      throw new BookmarkError(
        'Bookmark ID is required in route parameters.',
        400
      );
    ...
    logger.error(
      `Error adding tag to bookmark ${bookmarkId}:`,
      error
    );
    next(error);
  }
...
export const removeTagFromBookmark = async (
  ...
  try {
    ...
    if (!bookmarkId || !tagId) {
      throw new BookmarkError(
        'Bookmark ID and Tag ID are required in route parameters',
        400
      );
    }
...
export const getRecentBookmarks = async (
  ...
  try {
    ...
    const bookmarks = await bookmarkService.getRecentBookmarks(
      userId,
      limit
    );
    ...
    if (error instanceof z.ZodError) {
      logger.error(
        'Recent bookmarks query validation error:',
        error.errors
      );
    } else {
...
export const getPopularBookmarks = async (
  ...
  try {
    ...
    const bookmarks = await bookmarkService.getPopularBookmarks(
      userId,
      limit
    );
    res.status(200).json({ success: true, bookmarks });
    ...
    if (error instanceof z.ZodError) {
      logger.error(
        'Popular bookmarks query validation error:',
        error.errors
      );
    } else {
...
export const performBulkAction = async (
  ...
  try {
    ...
    const result = await bookmarkService.performBulkAction(
      userId,
      bulkActionData
    );
