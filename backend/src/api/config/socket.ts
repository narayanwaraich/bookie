...
export const initSocketIO = (
  ...
  logger.info(
    '[Socket.IO]: Server initialized and middleware applied.'
  );
  ...
  io.on('connection', (socket: Socket) => {
    // Use base Socket type here, cast later
    ...
    if (!authenticatedSocket.user) {
      logger.error(
        `[Socket.IO]: Connection event for unauthenticated socket ${authenticatedSocket.id}. Disconnecting.`
      );
    ...
    logger.info(
      `[Socket.IO]: Authenticated client connected: ${authenticatedSocket.id}, User: ${authenticatedSocket.user.id}`
    );

    // Join a room specific to the user
    authenticatedSocket.join(authenticatedSocket.user.id);
    logger.info(
      `[Socket.IO]: Socket ${authenticatedSocket.id} joined room ${authenticatedSocket.user.id}`
    );

    authenticatedSocket.on('disconnect', () => {
      logger.info(
        `[Socket.IO]: Client disconnected: ${authenticatedSocket.id}`
      );
    });
