...
const setRefreshTokenCookie = (res: any, token: string) => {
  ...
  try {
    ...
    logger.error(
      `Invalid refreshExpiresIn format in config: "${config.jwt.refreshExpiresIn}". Defaulting cookie maxAge to 7 days.`
    );
...
export const authTrpcRouter = router({
  ...
  /** Refresh Token */
  refreshToken: publicProcedure.query(async ({ ctx }) => {
    ...
    try {
      ...
      const decodedPayload = decode(accessToken) as {
        sub?: string;
        id?: string;
      } | null; // Use imported decode, check for id or sub
      const userIdFromToken =
        decodedPayload?.id || decodedPayload?.sub; // Prefer id if present
      if (!userIdFromToken) {
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
          message: 'Failed to decode user ID from new access token',
        });
      }
      const user = await authService.getAuthenticatedUser(
        userIdFromToken
      ); // Fetch user based on ID from token
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        if (error instanceof AuthError) {
          logger.warn(
            `Password reset request potentially failed for ${input.email}: ${error.message}`
          );
          return {
            success: true,
            message:
