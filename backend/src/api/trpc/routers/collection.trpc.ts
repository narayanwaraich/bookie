...
export const collectionTrpcRouter = router({
  ...
  getById: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      try {
        ...
        return await collectionService.getCollectionById(
          id,
          userId,
          queryParams
        );
  ...
  getByPublicLink: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        return await collectionService.getPublicCollectionByLink(
          input.link
        );
      } catch (error: any) {
        if (
        ...
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
          message: 'Failed to fetch public collection',
          cause: error,
        });
      }
    }),
  ...
  addBookmark: protectedProcedure
    .input(
      z
        ...
        .merge(addBookmarkToCollectionSchema)
    )
    .mutation(async ({ ctx, input }) => {
      const userId = ctx.user.id;
      try {
        return await collectionService.addBookmarkToCollection(
          input.collectionId,
          input.bookmarkId,
          userId
        );
        ...
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
          message: 'Failed to add bookmark to collection',
          cause: error,
        });
      }
    }),
  ...
  removeBookmark: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
          message: 'Failed to remove bookmark from collection',
          cause: error,
        });
      }
    }),
  ...
  addCollaborator: protectedProcedure
    .input(
      z
        .object({ collectionId: z.string().uuid() })
        .merge(addCollectionCollaboratorSchema)
    )
    .mutation(async ({ ctx, input }) => {
      const ownerId = ctx.user.id;
      try {
        return await collectionService.addCollaborator(
          input.collectionId,
          ownerId,
          input.userId,
          input.permission
  ...
  updateCollaborator: protectedProcedure
    .input(
      z
        ...
        .merge(updateCollectionCollaboratorSchema)
    )
    .mutation(async ({ ctx, input }) => {
      const ownerId = ctx.user.id;
      try {
        return await collectionService.updateCollaboratorPermission(
          input.collectionId,
          ownerId,
