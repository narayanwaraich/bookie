...
export const bookmarkTrpcRouter = router({
      ...
      try {
        ...
        if (!newBookmark)
          throw new TRPCError({
            ...
            message: 'Failed to retrieve created bookmark details.',
          });
        return newBookmark;
      } catch (error: any) {
  ...
  getById: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const userId = ctx.user.id;
      try {
        return await bookmarkService.getBookmarkById(
          input.id,
          userId
        );
      } catch (error: any) {
  ...
  update: protectedProcedure
    .input(
      z.object({ id: z.string().uuid() }).merge(updateBookmarkSchema)
    ) // Combine ID param with update data schema
    .mutation(async ({ ctx, input }) => {
      const { id, ...updateData } = input; // Separate ID from update data
      const userId = ctx.user.id;
      try {
        return await bookmarkService.updateBookmark(
          id,
          userId,
          updateData
        );
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const userId = ctx.user.id;
      try {
        return await bookmarkService.getRecentBookmarks(
          userId,
          input.limit
        );
      } catch (error: any) {
  ...
  getPopular: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const userId = ctx.user.id;
      try {
        return await bookmarkService.getPopularBookmarks(
          userId,
          input.limit
        );
      } catch (error: any) {
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
          message: 'Failed to fetch popular bookmarks',
          cause: error,
        });
      }
    }),
  ...
  checkUrlExists: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const userId = ctx.user.id;
      try {
        return await bookmarkService.checkBookmarkUrlExists(
          userId,
          input.url
        );
      } catch (error: any) {
  ...
  addTag: protectedProcedure
    .input(
      z
        ...
        .merge(addTagToBookmarkBodySchema)
    ) // Combine bookmarkId param and tagId body
    .mutation(async ({ ctx, input }) => {
      const userId = ctx.user.id;
      try {
      ...
      try {
        ...
        if (
          ...
          throw new TRPCError({
            ...
            message: error.message,
            cause: error,
          }); // Use BAD_REQUEST for client errors
        }
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
