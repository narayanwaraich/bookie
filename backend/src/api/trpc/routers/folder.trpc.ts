...
export const folderTrpcRouter = router({
  ...
  getById: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      try {
        ...
        if (error instanceof FolderError && error.statusCode === 404)
          throw new TRPCError({
            code: 'NOT_FOUND',
            message: error.message,
            cause: error,
          });
        if (error instanceof FolderError && error.statusCode === 403)
          throw new TRPCError({
            code: 'FORBIDDEN',
            message: error.message,
            cause: error,
  ...
  update: protectedProcedure
    .input(
      z.object({ id: z.string().uuid() }).merge(updateFolderSchema)
    )
    .mutation(async ({ ctx, input }) => {
      const { id, ...updateData } = input;
      const userId = ctx.user.id;
      try {
        return await folderService.updateFolder(
          id,
          userId,
          updateData
        );
  ...
  delete: protectedProcedure
    .input(
      z
        ...
        .merge(deleteFolderQuerySchema)
    ) // Combine ID param and query options
    .mutation(async ({ ctx, input }) => {
  ...
  getBookmarks: protectedProcedure
    .input(
      z
        ...
        .merge(getBookmarksInContainerQuerySchema)
    )
    .query(async ({ ctx, input }) => {
      ...
      try {
        ...
        return await folderService.getBookmarksByFolder(
          folderId,
          userId,
          queryParams
        );
        ...
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
          message: 'Failed to fetch bookmarks for folder',
          cause: error,
        });
      }
    }),
  ...
  addBookmark: protectedProcedure
    .input(
      z
        .object({ folderId: z.string().uuid() })
        .merge(addBookmarkToFolderBodySchema)
    )
    .mutation(async ({ ctx, input }) => {
      const userId = ctx.user.id;
      try {
  ...
  removeBookmark: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
          message: 'Failed to remove bookmark from folder',
          cause: error,
        });
      }
    }),
  ...
  addCollaborator: protectedProcedure
    .input(
      z
        .object({ folderId: z.string().uuid() })
        .merge(addFolderCollaboratorSchema)
    )
    .mutation(async ({ ctx, input }) => {
      const ownerId = ctx.user.id;
      try {
        return await folderService.addCollaborator(
          input.folderId,
          ownerId,
          input.userId,
          input.permission
  ...
  updateCollaborator: protectedProcedure
    .input(
      z
        ...
        .merge(updateFolderCollaboratorSchema)
    )
    .mutation(async ({ ctx, input }) => {
      const ownerId = ctx.user.id;
      try {
        return await folderService.updateCollaboratorPermission(
          input.folderId,
          ownerId,
