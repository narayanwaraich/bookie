    ...
    try {
      ...
      if (error instanceof ZodError) {
        ...
        const errorMessages = error.errors.map((err) => ({
          field: err.path
            .join('.')
            .replace(/^body\.|^query\.|^params\./, ''), // Remove prefix for clarity
          message: err.message,
        }));
        logger.warn(
          `[Validation Middleware] Input validation failed for ${
            req.method
          } ${req.originalUrl}: ${JSON.stringify(errorMessages)}`
        );
      ...
      logger.error(
        '[Validation Middleware] Unexpected error during validation:',
        error
      );
